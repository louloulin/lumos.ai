...
async fn main() {
    ...
    match info_tool.execute(params, context.clone(), &options).await {
        Ok(result) => {
            ...
            println!("  时长: {} ({}秒)", result["duration_formatted"], result["duration_seconds"]);
    ...
    let conversions = vec![
        ("WAV → MP3", "/path/to/input.wav", "/path/to/output.mp3", "mp3", 320),
        ("MP3 → AAC", "/path/to/input.mp3", "/path/to/output.aac", "aac", 256),
        ("WAV → FLAC", "/path/to/input.wav", "/path/to/output.flac", "flac", 192),
    ];
        ...
        match convert_tool.execute(params, context.clone(), &options).await {
            Ok(result) => {
                if result["success"] == true {
                    println!("  ✅ 转换成功:");
                    println!("    输入: {} ({} MB)", result["input_format"], result["input_size"]);
                    println!("    输出: {} ({} MB)", result["output_format"], result["output_size"]);
                    println!("    比特率: {} kbps", result["bit_rate"]);
                    println!("    采样率: {} Hz", result["sample_rate"]);
    ...
    for (desc, params) in processing_scenarios {
        ...
        match process_tool.execute(params, context.clone(), &options).await {
            Ok(result) => {
                println!("  ✅ 处理结果:");
