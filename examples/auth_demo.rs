...
use lumosai_core::auth::{
    ...
    jwt::JWTManager,
    multi_tenant::{SubscriptionPlan, TenantManager},
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let user = auth_manager
        .create_user("demo@lumos.ai", "secure_password", None)
        .await?;
    println!("✅ Created user: {}", user.email);

    // Authenticate user
    let auth_token = auth_manager
        .authenticate("demo@lumos.ai", "secure_password")
        .await?;
    println!(
    ...
    let api_key = api_manager
        .generate_key(
            user_id,
            "demo-key",
            vec!["agents:read".to_string(), "tools:execute".to_string()],
        )
        .await?;
    ...
    let startup_tenant = tenant_manager
        .create_tenant(
        ...
        .await?;
    println!(
        "✅ Created startup tenant: {} (Plan: {:?})",
        startup_tenant.name, startup_tenant.subscription_plan
    );
    ...
    let enterprise_tenant = tenant_manager
        ...
        .await?;
    println!(
        "✅ Created enterprise tenant: {} (Plan: {:?})",
        enterprise_tenant.name, enterprise_tenant.subscription_plan
    );

    // Check resource limits
    let startup_can_add_users = tenant_manager
        .check_resource_limit(&startup_tenant.id, "users", 20)
        .await?;
    let startup_cannot_add_many = tenant_manager
        .check_resource_limit(&startup_tenant.id, "users", 50)
        .await?;
    println!(
        "✅ Startup can add 20 users: {}, can add 50 users: {}",
        startup_can_add_users, startup_cannot_add_many
    );

    let enterprise_can_add_many = tenant_manager
        .check_resource_limit(&enterprise_tenant.id, "users", 500)
        .await?;
    println!(
        "✅ Enterprise can add 500 users: {}",
        enterprise_can_add_many
    );

    // Set custom domain
    tenant_manager
        .set_tenant_domain(&enterprise_tenant.id, "enterprise.lumos.ai".to_string())
        .await?;
    println!("✅ Set custom domain for enterprise tenant");

    let found_tenant = tenant_manager
        .get_tenant_by_domain("enterprise.lumos.ai")
        .await;
    println!("✅ Found tenant by domain: {}", found_tenant.unwrap().name);

    // Update usage
    tenant_manager
        .update_usage(&startup_tenant.id, "users", 15)
        .await?;
    tenant_manager
        .update_usage(&startup_tenant.id, "agents", 25)
        .await?;

    let usage = tenant_manager.get_usage(&startup_tenant.id).await.unwrap();
    println!(
        "✅ Startup tenant usage - Users: {}, Agents: {}",
        usage.current_users, usage.current_agents
    );
    ...
    let invalid_auth = auth_manager
        .authenticate("invalid@email.com", "wrong_password")
        .await;
    println!(
    ...
    println!(
        "✅ Available roles: {:?}",
        all_roles.iter().map(|r| &r.name).collect::<Vec<_>>()
    );
