...
async fn demo_advanced_agent() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let tech_response = advanced_agent
        .generate_simple("请详细解释 Rust 中的所有权概念，包括其核心原理和优势")
        .await?;
...
async fn demo_conversation() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let response1 = conversation_agent
        .generate_simple("我叫张三，今年25岁")
        .await?;
    println!("第1轮 - 用户: 我叫张三，今年25岁");
    println!("第1轮 - AI: {}", response1);

    let response2 = conversation_agent
        .generate_simple("我的爱好是编程和阅读")
        .await?;
    println!("\n第2轮 - 用户: 我的爱好是编程和阅读");
    println!("第2轮 - AI: {}", response2);

    let response3 = conversation_agent
        .generate_simple("请告诉我，你还记得我的名字和年龄吗？")
        .await?;
...
) -> std::result::Result<Arc<dyn lumosai_core::llm::LlmProvider>, Box<dyn std::error::Error>> {
