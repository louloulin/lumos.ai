...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let subscription =
        subscription_manager.create_subscription(tenant_id, "professional".to_string())?;
    ...
    for (resource_type, stats) in &usage_stats.usage_by_resource {
        println!(
            "   • {}: {} {} (成本: ${:.2})",
            resource_type, stats.total_usage, stats.unit, stats.total_cost
        );
    }
    println!();
    ...
    println!(
        "   • 存储: {}",
        if storage_limit_check.allowed {
            "✅ 正常"
        } else {
            "⚠️ 超限"
        }
    ...
    // 生成计费项目
    let billing_items =
        billing_engine.calculate_usage_cost(&tenant_id, &usage_stats, &subscription)?;
