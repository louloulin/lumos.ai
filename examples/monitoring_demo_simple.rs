...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in 0..5 {
        ...
        let mut agent_metrics =
            AgentMetrics::new(format!("ai-assistant-{}", i % 2), execution_context);
        ...
        metrics_collector
            ...
            .await?;
        println!(
            "✅ Recorded agent execution metrics #{} (Agent: ai-assistant-{})",
            i + 1,
            i % 2
        );
    }
    ...
    for (i, (tool_name, base_time, success)) in tools.iter().enumerate() {
        ...
        metrics_collector
            ...
            .await?;
        println!(
            "✅ Recorded tool execution metrics for {} ({}ms, success: {})",
            tool_name,
            base_time + (i * 50) as u64,
            success
        );
    }
    ...
    for (i, (operation, base_time, success, key, size)) in operations.iter().enumerate() {
        ...
        metrics_collector
            ...
            .await?;
        println!(
            "✅ Recorded memory operation metrics for {} ({}ms, success: {})",
            operation,
            base_time + (i * 2) as u64,
            success
        );
    }
