...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ Quick API: {} agents created in {:?}",
        iterations, quick_api_duration
    );
    println!(
        "   Average: {:?} per agent",
        quick_api_duration / iterations
    );
    ...
    println!(
        "✅ Builder API: {} agents created in {:?}",
        iterations, builder_api_duration
    );
    println!(
        "   Average: {:?} per agent",
        builder_api_duration / iterations
    );
    ...
    println!(
        "✅ Smart Defaults: {} agents created in {:?}",
        iterations, smart_defaults_duration
    );
    println!(
        "   Average: {:?} per agent",
        smart_defaults_duration / iterations
    );
    ...
    println!(
        "✅ Web Agents: {} agents created in {:?}",
        web_iterations, web_agent_duration
    );
    println!(
        "   Average: {:?} per agent",
        web_agent_duration / web_iterations
    );
    ...
    println!(
        "✅ Multi-Tool Agents: {} agents created in {:?}",
        web_iterations, multi_tool_duration
    );
    println!(
        "   Average: {:?} per agent",
        multi_tool_duration / web_iterations
    );
    ...
    println!(
        "✅ Memory Usage: {} KB for 100 agents",
        end_memory - start_memory
    );
    println!(
        "   Average: {} KB per agent",
        (end_memory - start_memory) / 100
    );
    ...
    let quick_vs_builder = if quick_api_duration < builder_api_duration {
        let improvement = ((builder_api_duration.as_nanos() as f64
            - quick_api_duration.as_nanos() as f64)
            / builder_api_duration.as_nanos() as f64)
            * 100.0;
        format!("{:.1}% faster", improvement)
    } else {
        let degradation = ((quick_api_duration.as_nanos() as f64
            - builder_api_duration.as_nanos() as f64)
            / builder_api_duration.as_nanos() as f64)
            * 100.0;
    ...
    let smart_vs_builder = if smart_defaults_duration < builder_api_duration {
        let improvement = ((builder_api_duration.as_nanos() as f64
            - smart_defaults_duration.as_nanos() as f64)
            / builder_api_duration.as_nanos() as f64)
            * 100.0;
        format!("{:.1}% faster", improvement)
    } else {
        let degradation = ((smart_defaults_duration.as_nanos() as f64
            - builder_api_duration.as_nanos() as f64)
            / builder_api_duration.as_nanos() as f64)
            * 100.0;
        format!("{:.1}% slower", degradation)
...
mod tests {
    ...
    async fn test_api_performance_consistency() {
        ...
        assert_eq!(
            quick_agent.get_instructions(),
            builder_agent.get_instructions()
        );
    }
