use lumosai_core::llm::{LlmOptions, LlmProvider, Message, QwenApiType, QwenProvider, Role};
...
async fn test_dashscope_api(api_key: &str, model: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = provider
        .generate("你好，请简单介绍一下你自己。", &LlmOptions::default())
        .await;
...
async fn test_openai_compatible_api(
    api_key: &str,
    model: &str,
    ...
    let stream_result = provider
        .generate_stream("请写一首关于人工智能的短诗。", &options)
        .await;
