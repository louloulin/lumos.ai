...
async fn test_enterprise_integration() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    for scenario in enterprise_scenarios {
        ...
        let response = integration_agent
            .generate(&messages, &Default::default())
            .await?;
