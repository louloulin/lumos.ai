    ...
    match expression.as_str() {
        ...
        _ => {
            ...
            if let Ok(result) = expression.parse::<f64>() {
                ...
                Err(lumosai_core::error::Error::Tool(
                    format!("无法计算表达式: {}", expression)
                ))
            }
        }
    }
...
async fn validate_data(
    ...
    let is_valid = match data_type.as_str() {
        ...
        "phone" => {
            if strict {
                data.chars().all(|c| c.is_ascii_digit() || c == '-' || c == ' ') && data.len() >= 10
            } else {
...
async fn main() -> Result<()> {
    ...
    for param in &schema.parameters {
        println!("  - {}: {} ({})", param.name, param.description, param.r#type);
    }
