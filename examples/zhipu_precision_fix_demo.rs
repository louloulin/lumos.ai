use lumosai_core::agent::{types::AgentGenerateOptions, Agent, AgentBuilder};
use lumosai_core::llm::{
    types::{user_message, Temperature},
    LlmOptions, ZhipuProvider,
};
use std::env;
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, temp) in temperatures {
        ...
        // 创建带有特定温度的选项
        let options = LlmOptions::new().with_temperature_precise(temp);
