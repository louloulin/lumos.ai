...
impl EmbeddingProvider for SimpleEmbedder {
    async fn embed_text(&self, text: &str) -> lomusai_rag::error::Result<Vec<f32>> {
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let documents = vec![
        ...
        "Lomusai provides tools for building modern AI applications.",
    ];
...
async fn run_retrieval_demo(
    retriever: &mut impl lomusai_rag::retriever::VectorStore,
    embedder: Arc<dyn EmbeddingProvider>,
    documents: &[&str],
) -> Result<(), Box<dyn Error>> {
