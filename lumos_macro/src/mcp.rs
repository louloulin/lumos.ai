...
impl Parse for DiscoveryConfig {
    fn parse(input: ParseStream) -> syn::Result<Self> {
            ...
            match key.to_string().as_str() {
                ...
                _ => {
                    return Err(syn::Error::new(
                        key.span(),
                        "Unknown field in discovery config",
                    ))
                }
            }
        }
        ...
        let endpoints = endpoints.ok_or_else(|| {
            syn::Error::new(
                content.span(),
                "Missing 'endpoints' field in discovery config",
            )
        })?;
...
impl Parse for AuthConfig {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        let auth_type = auth_type.ok_or_else(|| {
            syn::Error::new(content.span(), "Missing 'type' field in auth config")
        })?;

        Ok(AuthConfig { auth_type, key_env })
    }
}
...
impl Parse for McpClientDef {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        while !content.is_empty() {
            ...
            match key.to_string().as_str() {
                ...
                _ => {
                    return Err(syn::Error::new(
                        key.span(),
                        "Unknown field in MCP client definition",
                    ))
                }
            }
        }
        ...
        let discovery = discovery.ok_or_else(|| {
            syn::Error::new(
                content.span(),
                "Missing 'discovery' field in MCP client definition",
            )
        })?;

        Ok(McpClientDef { discovery, tools })
    }
}
