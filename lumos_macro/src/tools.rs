...
use syn::{
    ...
    Expr, ExprClosure, Ident, LitBool, LitStr, Token,
};
...
impl Parse for ToolParameter {
    fn parse(input: ParseStream) -> syn::Result<Self> {
            ...
            match key.to_string().as_str() {
                ...
                _ => {
                    return Err(syn::Error::new(
                        key.span(),
                        "Unknown field in parameter definition",
                    ))
                }
            }
        }
        ...
        let name = name.ok_or_else(|| {
            syn::Error::new(
                content.span(),
                "Missing 'name' field in parameter definition",
            )
        })?;
        let description = description.ok_or_else(|| {
            syn::Error::new(
                content.span(),
                "Missing 'description' field in parameter definition",
            )
        })?;
...
impl Parse for ToolDef {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        while !content.is_empty() {
            ...
            match key.to_string().as_str() {
                ...
                _ => {
                    return Err(syn::Error::new(
                        key.span(),
                        "Unknown field in tool definition",
                    ))
                }
            }
        }
        ...
        let name = name.ok_or_else(|| {
            syn::Error::new(content.span(), "Missing 'name' field in tool definition")
        })?;
        let description = description.ok_or_else(|| {
            syn::Error::new(
                content.span(),
                "Missing 'description' field in tool definition",
            )
        })?;
        ...
        let parameters_content = parameters_content.ok_or_else(|| {
            syn::Error::new(
                content.span(),
                "Missing 'parameters' field in tool definition",
            )
        })?;
        ...
        let handler = handler.ok_or_else(|| {
            syn::Error::new(content.span(), "Missing 'handler' field in tool definition")
        })?;
