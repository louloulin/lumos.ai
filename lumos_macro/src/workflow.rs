...
impl Parse for WorkflowStep {
    fn parse(input: ParseStream) -> syn::Result<Self> {
            ...
            match key.to_string().as_str() {
                ...
                _ => {
                    return Err(syn::Error::new(
                        key.span(),
                        "Unknown field in workflow step",
                    ))
                }
            }
        }
        ...
        let name = name.ok_or_else(|| {
            syn::Error::new(content.span(), "Missing 'name' field in workflow step")
        })?;
        let agent = agent.ok_or_else(|| {
            syn::Error::new(content.span(), "Missing 'agent' field in workflow step")
        })?;
...
impl Parse for WorkflowDef {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        let name = name.ok_or_else(|| {
            syn::Error::new(
                content.span(),
                "Missing 'name' field in workflow definition",
            )
        })?;
        ...
        let steps_content = steps_content.ok_or_else(|| {
            syn::Error::new(
                content.span(),
                "Missing 'steps' field in workflow definition",
            )
        })?;
