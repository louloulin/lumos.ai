...
impl WasmAgent {
    ...
    pub fn get_config(&self) -> Result<JsValue, JsValue> {
        ...
        js_sys::Reflect::set(
            &runtime_obj,
            &"timeoutSeconds".into(),
            &(config.runtime.timeout_seconds as f64).into(),
        )?;
        js_sys::Reflect::set(
            &runtime_obj,
            &"maxRetries".into(),
            &(config.runtime.max_retries as f64).into(),
        )?;
        js_sys::Reflect::set(
            &runtime_obj,
            &"concurrencyLimit".into(),
            &(config.runtime.concurrency_limit as f64).into(),
        )?;
        js_sys::Reflect::set(
            &runtime_obj,
            &"enableLogging".into(),
            &config.runtime.enable_logging.into(),
        )?;
        js_sys::Reflect::set(
            &runtime_obj,
            &"logLevel".into(),
            &config.runtime.log_level.into(),
        )?;
...
impl WasmTool {
    ...
    pub fn execute(&self, parameters: &JsValue) -> Result<JsValue, JsValue> {
        ...
        let parameters: serde_json::Value =
            serde_json::from_str(&params_str).map_err(|e| JsValue::from_str(&e.to_string()))?;
        ...
        js_sys::Reflect::set(
            ...
            &(result.execution_time_ms as f64).into(),
        )?;

        // 结果数据转换
        let result_str =
            serde_json::to_string(&result.result).map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    pub fn tool_calls(&self) -> Result<JsValue, JsValue> {
        ...
        for tool_call in &self.inner.tool_calls {
            ...
            js_sys::Reflect::set(
                &js_call,
                &"executionTimeMs".into(),
                &(tool_call.execution_time_ms as f64).into(),
            )?;
