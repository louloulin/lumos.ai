...
pub async fn run(
    ...
    println!(
        "{}",
        format!("输出目录: {}", output_dir.display()).bright_blue()
    );

    // 创建输出目录（如果不存在）
    if !output_dir.exists() {
        fs::create_dir_all(&output_dir).map_err(|e| {
            CliError::io_string(format!("无法创建输出目录: {}", output_dir.display()), e)
        })?;
    }
        ...
        Some(specified_agents) => {
            ...
            for agent in &specified_agents {
                if !available_agents.contains(agent) {
                    return Err(CliError::invalid_input_string(format!(
                        "指定的代理 '{}' 不存在。请检查代理名称或添加该代理",
                        agent
                    )));
                }
            }
    ...
    if agents_to_process.is_empty() {
        ...
        println!(
            "{}",
            "请先创建至少一个代理，或指定有效的代理".bright_yellow()
        );
        return Ok(());
    }
    ...
    println!(
        "{}",
        format!("找到 {} 个代理:", agents_to_process.len()).bright_blue()
    );
    ...
    println!(
        "{}",
        format!("API文件位于: {}", output_dir.display()).bright_green()
    );
...
fn find_agents(agents_dir: &Path) -> CliResult<Vec<String>> {
    ...
    for entry in fs::read_dir(agents_dir).map_err(|e| {
        CliError::io_string(format!("无法读取代理目录: {}", agents_dir.display()), e)
    })? {
...
fn generate_api_module(output_dir: &Path, agents: &[String]) -> CliResult<()> {
    ...
    for agent in agents {
        content.push_str(&format!(
            "    cfg.service(web::scope(\"/{}\").configure({}::configure_routes));\n",
            agent, agent
        ));
    }
    ...
    println!(
        "{}",
        format!("生成API模块: {}", mod_path.display()).bright_green()
    );
...
fn generate_agent_api(output_dir: &Path, agent_name: &str, project_dir: &Path) -> CliResult<()> {
    ...
    content.push_str("    \n");
    content.push_str(
        "    match agent.process_message(&req.message, req.conversation_id.clone()).await {\n",
    );
    ...
    // 写入文件
    fs::write(&agent_api_path, content).map_err(|e| {
        CliError::io_string(format!("无法写入文件: {}", agent_api_path.display()), e)
    })?;
...
mod tests {
    ...
    async fn test_run_with_no_agents() {
        ...
        let result = run(Some(temp_path.to_path_buf()), None, None).await;
    ...
    async fn test_run_with_invalid_specified_agent() {
        ...
        if let Err(e) = result {
            assert!(e
                .to_string()
                .contains("指定的代理 'non_existent_agent' 不存在"));
        }
    }
