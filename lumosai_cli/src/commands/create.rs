...
pub async fn run(
    ...
    let components = match components {
        ...
        None => {
            ...
            selections
                .iter()
                .map(|&i| choices[i].split(" - ").next().unwrap().to_string())
                .collect()
        }
    };
    ...
    println!(
        "{}",
        format!("项目 '{}' 创建成功!", project_name).bright_green()
    );
...
async fn install_dependencies(options: &CreateOptions) -> CliResult<()> {
    ...
    if status.success() {
        ...
        println!(
            "{}",
            "依赖安装可能出现问题，请稍后手动运行 'cargo build'".bright_yellow()
        );
    }
