...
/// 部署Lumos AI应用
pub async fn run(project_dir: Option<PathBuf>, target: &str) -> CliResult<()> {
    // 确定项目目录
    ...
    if !is_lumos_project(&project_dir) {
        ...
        println!(
            "{}",
            "如果这是错误的，请确认项目中包含lumosai依赖".bright_yellow()
        );
    }
...
async fn deploy_local(project_dir: &Path, build_dir: &Path) -> CliResult<()> {
    ...
    println!(
        "{}",
        format!("应用已部署到: {}", deploy_dir.display()).bright_green()
    );
    ...
    println!(
        "{}",
        format!("  cd {} && ./{}", deploy_dir.display(), project_name).bright_cyan()
    );
...
"#;
    ...
    println!(
        "{}",
        format!("创建Dockerfile: {}", dockerfile_path.display()).bright_green()
    );
    ...
    println!(
        "{}",
        format!("Docker镜像构建成功: {}", image_tag).bright_green()
    );
    println!("{}", "可以通过以下命令运行容器:".bright_blue());
    println!(
        "{}",
        format!("  docker run -p 3000:3000 {}", image_tag).bright_cyan()
    );
