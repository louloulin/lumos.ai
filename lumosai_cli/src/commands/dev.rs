...
use crate::server::{api_server, ui_server};
...
pub async fn run(options: DevOptions) -> CliResult<()> {
    ...
    if let Some(tools_dir) = &options.tools {
        println!(
            "{}",
            format!("自定义工具目录: {}", tools_dir.display()).bright_blue()
        );
    }
            ...
            while running.load(Ordering::SeqCst) {
                ...
                match api_server::start_server(options.port, project_dir.clone(), None).await {
                    ...
                    Err(e) => {
                        println!(
                            "{}",
                            format!("API服务器启动失败: {}，将在5秒后重试", e).bright_red()
                        );
            ...
            while running.load(Ordering::SeqCst) {
                ...
                .await
                {
                    ...
                    Err(e) => {
                        println!(
                            "{}",
                            format!("UI服务器启动失败: {}，将在5秒后重试", e).bright_red()
                        );
                        sleep(Duration::from_secs(5)).await;
                    }
                }
...
async fn start_file_watcher(
    ...
    // 可能的其他目录，如果存在也添加到监视
    let dirs_to_watch = [project_dir.join("config"), project_dir.join("assets")];
    ...
    println!(
        "{}",
        format!("正在监视目录: {}", src_dir.display()).bright_blue()
    );
