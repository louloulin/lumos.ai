...
pub async fn run(options: MonitoringOptions) -> CliResult<()> {
    ...
    if options.enable_realtime {
        ...
        println!(
            "{}",
            format!("刷新间隔: {} 秒", options.refresh_interval).bright_blue()
        );
    } else {
...
pub async fn health_check(port: u16) -> CliResult<()> {
    ...
    println!(
        "{}",
        format!("检查监控服务器健康状态: {}", url).bright_blue()
    );
        ...
        Ok(response) => {
            if response.status().is_success() {
                let status: serde_json::Value =
                    response.json().await.map_err(|e| CliError::from(e))?;
                ...
                println!(
                    "{}",
                    format!("监控服务器返回错误状态: {}", response.status()).bright_red()
                );
            }
        }
        Err(e) => {
