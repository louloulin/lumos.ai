...
use crate::util::{ensure_dir_exists, is_lumos_project};
...
pub async fn run(options: VisualizeOptions) -> CliResult<()> {
    ...
    if !is_lumos_project(&project_dir) {
        println!(
            "{}",
            "警告: 当前目录不是标准的Lumosai项目目录结构".bright_yellow()
        );
    }
    ...
    println!(
        "{}",
        format!("可视化图表已生成: {}", output_path.display()).bright_green()
    );
...
async fn generate_workflow_visualization(
    ...
    // 1. 生成DOT文件
    let dot_file =
        generate_workflow_dot_file(project_dir, workflow_id, include_performance).await?;
...
async fn generate_agent_dot_file(
    ...
    let agent_dir = project_dir.join("src").join("agents").join(agent_id);
    let dot_file_path = project_dir
        .join("visualizations")
        .join(format!("agent-{}.dot", agent_id));
    ...
    dot_content.push_str(&format!(
        "  agent [label=\"{}\", fillcolor=lightgreen];\n",
        agent_id
    ));
    ...
    for (i, tool) in tools.iter().enumerate() {
        dot_content.push_str(&format!(
            "  tool{} [label=\"Tool: {}\", fillcolor=lightyellow];\n",
            i, tool
        ));
    ...
    for (i, llm) in llms.iter().enumerate() {
        dot_content.push_str(&format!(
            "  llm{} [label=\"LLM: {}\", fillcolor=lightpink];\n",
            i, llm
        ));
    ...
    for (i, func) in functions.iter().enumerate() {
        dot_content.push_str(&format!(
            "  func{} [label=\"Function: {}\", fillcolor=lightgrey];\n",
            i, func
        ));
...
async fn generate_workflow_dot_file(
    ...
    let dot_file_path = project_dir
        .join("visualizations")
        .join(format!("workflow-{}.dot", workflow_id));
    ...
    dot_content.push_str("  node [shape=box, style=filled, fillcolor=lightblue];\n");
    dot_content.push_str(&format!(
        "  workflow [label=\"{}\", fillcolor=lightgreen];\n",
        workflow_id
    ));
    ...
    for (i, step) in steps.iter().enumerate() {
        dot_content.push_str(&format!(
            "  step{} [label=\"Step {}: {}\", fillcolor=lightyellow];\n",
            i,
            i + 1,
            step
        ));
...
async fn convert_dot_to_format(
    ...
    if !check_graphviz().await {
        return Err(CliError::dependency(
            "Graphviz",
            "请安装Graphviz以生成可视化图表",
        ));
    }
    ...
    let status = cmd
        .status()
        .await
        .map_err(|e| CliError::io(format!("执行Graphviz dot命令失败: {}", e).as_str(), e))?;
...
fn parse_agent_code(code: &str) -> (Vec<String>, Vec<String>, Vec<String>) {
    ...
    for line in code.lines() {
        if line.contains("use") && line.contains("tool") {
            ...
            && (line.contains("llm") || line.contains("openai") || line.contains("anthropic"))
        {
...
/// 生成交互式HTML可视化
async fn generate_interactive_html(dot_file: &Path, output_path: &Path) -> CliResult<()> {
    // 读取DOT文件内容
    let dot_content = fs::read_to_string(dot_file).map_err(|e| CliError::io_error(e, dot_file))?;
...
"##,
    ...
    // 写入HTML文件
    fs::write(output_path, html_content).map_err(|e| CliError::io_error(e, output_path))?;
...
mod tests {
    ...
    fn test_extract_function_name() {
        ...
        assert_eq!(
            extract_function_name("    fn hello_world() -> String {"),
            Some("hello_world".to_string())
        );
        assert_eq!(
            extract_function_name("pub fn process(input: &str) -> Result<String, Error> {"),
    ...
    fn test_extract_name_from_use() {
        assert_eq!(
            extract_name_from_use("use lumosai::tools::calculator;"),
            Some("calculator".to_string())
        );
        assert_eq!(
            extract_name_from_use("use crate::agents::assistant::Agent;"),
