...
async fn main() -> CliResult<()> {
    ...
    match cli.command {
        ...
        Commands::Playground(options) => commands::playground::run(options).await,
        Commands::Api(args) => {
            // 将逗号分隔的字符串转换为 Vec<String>
            let agents = args
                .agents
            ...
            commands::api::run(args.project_dir, args.output, agents).await
        }
        Commands::Visualize(options) => commands::visualize::run(options).await,
        Commands::Monitoring(options) => commands::monitoring::run(options).await,
    }
}
...
async fn download_template(url: String, name: Option<String>) -> CliResult<()> {
    ...
    println!(
        "{}",
        format!("下载模板 '{}' 从 {}", template_name, url).bright_blue()
    );
    ...
    println!(
        "{}",
        format!("模板 '{}' 已下载", template_name).bright_green()
    );
