...
impl ApiServerConfig {
    // 创建新配置
    pub fn new(port: u16, project_dir: PathBuf, api_module_path: Option<PathBuf>) -> Self {
...
async fn api_info(data: web::Data<ApiServerConfig>) -> impl Responder {
    ...
    // 构建可用端点列表
    let mut endpoints = vec!["/api".to_string(), "/api/info".to_string()];
...
pub fn start_server(
    ...
    Box::pin(async move {
        ...
        // 创建配置
        let config = ApiServerConfig::new(port, project_dir.clone(), api_module_path);

        // 检查API模块是否存在
        if !check_api_module(&config) {
            println!(
                "{}",
                "警告: API模块不存在，只提供基本API信息".bright_yellow()
            );
        ...
        println!(
            "{}",
            format!("API模块路径: {}", config.api_module_path.display()).bright_blue()
        );
        println!(
            "{}",
        ...
        println!(
            "{}",
            format!("访问: http://localhost:{}/api/info", config.port).bright_green()
        );
