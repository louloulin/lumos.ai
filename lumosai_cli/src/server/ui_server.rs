...
pub fn find_ui_dir() -> CliResult<PathBuf> {
    ...
    // 否则使用相对于二进制的路径
    let exe_path = env::current_exe().map_err(|e| CliError::io("获取可执行文件路径失败", e))?;
...
pub fn find_playground_dir() -> CliResult<PathBuf> {
    ...
    println!(
        "{}",
        "警告: 找不到Playground目录，使用默认路径".bright_yellow()
    );
...
async fn server_info(data: web::Data<UiServerConfig>) -> impl Responder {
    ...
    let info = ServerInfo {
        ...
        save_history: if config.is_playground {
            Some(config.save_history)
        } else {
            None
        },
    };
...
async fn start_dev_server(config: UiServerConfig) -> CliResult<()> {
    ...
    // 设置项目目录
    cmd_env.insert(
        "LUMOS_PROJECT_DIR".to_string(),
        config.project_dir.to_string_lossy().to_string(),
    );
    ...
    if config.is_playground {
        ...
        cmd_env.insert(
            "LUMOS_SAVE_HISTORY".to_string(),
            config.save_history.to_string(),
        );
    }
    ...
    println!(
        "{}",
        format!("正在使用 {} 启动开发服务器...", cmd).bright_blue()
    );
    println!(
        "{}",
        format!("UI目录: {}", config.ui_dir.display()).bright_blue()
    );
    println!("{}", format!("端口: {}", config.port).bright_blue());

    let mut child = dev_command
        .spawn()
    ...
    if let Err(e) = open::that(format!("http://localhost:{}", config.port)) {
        ...
        println!(
            "{}",
            format!("请手动访问: http://localhost:{}", config.port).bright_green()
        );
    }
...
async fn start_production_server(config: UiServerConfig) -> CliResult<()> {
    ...
    println!(
        "{}",
        format!("静态资源目录: {}", static_dir.display()).bright_blue()
    );
    ...
    let app_factory = move || {
        ...
        App::new()
            ...
            .service(
                fs_web::Files::new("/", static_dir.clone())
    ...
    println!(
        "{}",
        format!("UI服务器已启动，监听地址: {}", config.get_bind_address()).bright_green()
    );
    ...
    if let Err(e) = open::that(format!("http://{}", config.get_bind_address())) {
        ...
        println!(
            "{}",
            format!("请手动访问: http://{}", config.get_bind_address()).bright_green()
        );
    }
    ...
    server
        .await
        .map_err(|e| CliError::io("HTTP服务器错误", e))?;
...
pub fn start_server(
    ...
    Box::pin(async move {
        ...
        // 创建配置
        let config = UiServerConfig::new(ui_dir, port, theme, dev_mode, api_url, project_dir);
