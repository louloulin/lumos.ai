...
impl Template {
    ...
    pub fn load(template_type: &TemplateType) -> CliResult<Self> {
        ...
        // 解析模板配置
        let template_json =
            fs::read_to_string(&config_path).map_err(|e| CliError::io_error(e, &config_path))?;

        let template: Template =
            serde_json::from_str(&template_json).map_err(|e| CliError::template_parse_error(e))?;
    ...
    fn get_template_dir() -> CliResult<PathBuf> {
        // 获取用户主目录
        let home_dir =
            dirs::home_dir().ok_or_else(|| CliError::Other("无法确定用户主目录".to_string()))?;
    ...
    pub fn apply(&self, project_name: &str, output_dir: &Path) -> CliResult<()> {
        ...
        for file in &self.files {
            ...
            // 写入文件内容
            fs::write(&file_path, content).map_err(|e| CliError::io_error(e, &file_path))?;

            println!(
                "{}",
                format!("创建文件: {}", file_path.display()).bright_green()
            );
        }
...
"#,
        ...
        // 写入Cargo.toml
        fs::write(&cargo_path, cargo_content).map_err(|e| CliError::io_error(e, &cargo_path))?;

        println!(
            "{}",
            format!("创建文件: {}", cargo_path.display()).bright_green()
        );
...
impl TemplateManager {
    ...
    pub fn download_template(&self, url: &str, name: &str) -> CliResult<()> {
        ...
        println!(
            "{}",
            format!("模板 {} 已下载到 {}", name, template_path.display()).bright_green()
        );
...
"#
        ...
        fs::write(&config_path, config_json).map_err(|e| CliError::io_error(e, &config_path))?;
    ...
    pub fn remove_template(&self, name: &str) -> CliResult<()> {
        ...
        // 删除模板目录
        fs::remove_dir_all(&template_path).map_err(|e| CliError::io_error(e, &template_path))?;
    ...
    pub fn create_project(
        ...
        let template = match Template::load(template_type) {
            ...
            Err(e) => {
                ...
                println!(
                    "{}",
                    format!("找不到 {} 模板: {}", template_type.name(), e).bright_yellow()
                );
...
"#,
        ...
        fs::write(&main_path, main_content).map_err(|e| CliError::io_error(e, &main_path))?;

        println!(
            "{}",
            format!("创建文件: {}", main_path.display()).bright_green()
        );
...
"#,
        ...
        fs::write(&readme_path, readme_content).map_err(|e| CliError::io_error(e, &readme_path))?;

        println!(
            "{}",
            format!("创建文件: {}", readme_path.display()).bright_green()
        );
