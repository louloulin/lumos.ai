...
pub fn find_project_root() -> CliResult<PathBuf> {
    let current_dir = env::current_dir().map_err(|e| CliError::io("无法获取当前目录", e))?;
...
pub fn ensure_dir_exists<P: AsRef<Path>>(path: P) -> CliResult<()> {
    ...
    if !path.exists() {
        fs::create_dir_all(path).map_err(|e| CliError::io_error(e, path))?;
    }
    Ok(())
}
...
pub fn copy_recursively<P: AsRef<Path>, Q: AsRef<Path>>(src: P, dst: Q) -> CliResult<()> {
    ...
    if src.is_dir() {
        if !dst.exists() {
            fs::create_dir_all(dst).map_err(|e| CliError::io_error(e, dst))?;
        }

        for entry in fs::read_dir(src).map_err(|e| CliError::io_error(e, src))? {
        ...
        fs::copy(src, dst).map_err(|e| {
            CliError::io_error(e, format!("{} -> {}", src.display(), dst.display()))
        })?;
    }
...
pub fn create_dir_all<P: AsRef<Path>>(path: P) -> CliResult<()> {
    fs::create_dir_all(&path).map_err(|e| CliError::io_error(e, &path.as_ref()))
}
...
pub fn copy_dir_all<P: AsRef<Path>, Q: AsRef<Path>>(from: P, to: Q) -> CliResult<()> {
    ...
    // 读取源目录
    for entry in fs::read_dir(from_path).map_err(|e| CliError::io_error(e, from_path))? {
        ...
        if from_entry.is_dir() {
            ...
            // 复制文件
            fs::copy(&from_entry, &to_entry).map_err(|e| CliError::io_error(e, &from_entry))?;
        }
    }
...
pub fn read_project_config(project_dir: &Path) -> CliResult<toml::Value> {
    ...
    // 读取配置文件内容
    let content =
        fs::read_to_string(&config_file).map_err(|e| CliError::io_error(e, &config_file))?;
