...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let standards = vec![ComplianceStandard::SOC2, ComplianceStandard::GDPR];
    ...
    println!(
        "  当前密钥ID: {}",
        security_status.encryption_status.current_key_id
    );
    println!(
        "  加密算法: {:?}",
        security_status.encryption_status.algorithm
    );
    println!(
        "  密钥年龄: {} 小时",
        security_status.encryption_status.key_age_hours
    );

    println!("\n🛡️ 零信任状态:");
    println!(
        "  活跃会话: {}",
        security_status.zero_trust_status.active_sessions
    );
    println!(
        "  受信设备: {}",
        security_status.zero_trust_status.trusted_devices
    );
    println!(
        "  活跃策略: {}",
        security_status.zero_trust_status.active_policies
    );
    ...
    println!(
        "  整体合规分数: {:.2}%",
        security_status.compliance_status.overall_compliance_score * 100.0
    );
    println!(
        "  活跃违规: {}",
        security_status.compliance_status.active_violations
    );
