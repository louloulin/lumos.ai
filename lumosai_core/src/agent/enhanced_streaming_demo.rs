...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let weather_tool = Tool {
        ...
        implementation: Arc::new(|args| {
            Box::pin(async move {
                ...
                Ok(format!(
                    "Weather in {}: 22°C, Sunny with light clouds. Humidity: 65%, Wind: 8 km/h SW",
                    location
                ))
            })
        }),
    ...
    let llm = Arc::new(MockLlmProvider::new_with_function_calling(
        mock_responses,
        true,
    ));
    ...
    let test_scenarios = vec![
        ...
        (
            "Mixed Query",
            "Show me the weather in Paris and calculate 10 * 5",
        ),
