...
use crate::llm::{AnthropicProvider, LlmProvider, OpenAiProvider, QwenProvider};
...
impl ModelResolver {
    ...
    fn get_api_key(&self, provider: &str) -> Result<String> {
        self.api_keys.get(provider).cloned().ok_or_else(|| {
...
mod tests {
    ...
    fn test_infer_provider() {
        ...
        assert_eq!(
            resolver.infer_provider("claude-3-sonnet").unwrap(),
            "anthropic"
        );
        assert_eq!(
            resolver.infer_provider("deepseek-chat").unwrap(),
            "deepseek"
        );
