...
pub trait AgentOrchestrator: Send + Sync {
    ...
    async fn get_execution_status(
        &self,
        session_id: &str,
    ) -> Result<HashMap<String, AgentExecutionState>>;
}
...
impl BasicOrchestrator {
    ...
    async fn execute_sequential(
        ...
        for agent_id in &session.task.participants {
            if let Some(agent) = session.agents.get(agent_id) {
                // 更新状态为运行中
                session
                    .update_agent_state(agent_id, AgentExecutionState::Running)
                    .await;

                // 执行Agent
                match self
                    .execute_single_agent(agent.clone(), &session.task.input)
                    .await
                {
                    Ok(result) => {
                        results.insert(agent_id.clone(), result.clone());
                        session
                            .update_agent_state(agent_id, AgentExecutionState::Completed(result))
                            .await;
                    }
                    Err(e) => {
                        ...
                        session
                            .update_agent_state(
                                agent_id,
                                AgentExecutionState::Failed(error_msg.clone()),
                            )
                            .await;
    ...
    async fn execute_parallel(
        ...
        for handle in handles {
            match handle.await {
                Ok((agent_id, result)) => match result {
                    Ok(value) => {
                        results.insert(agent_id.clone(), value.clone());
                        session
                            .update_agent_state(&agent_id, AgentExecutionState::Completed(value))
                            .await;
                    }
                    Err(e) => {
                        let error_msg = e.to_string();
                        session
                            .update_agent_state(&agent_id, AgentExecutionState::Failed(error_msg))
                            .await;
                    }
                },
                Err(e) => {
...
impl AgentOrchestrator for BasicOrchestrator {
    async fn execute_collaboration(
        ...
        match &session.task.pattern {
            OrchestrationPattern::Sequential => self.execute_sequential(session).await,
            OrchestrationPattern::Parallel => self.execute_parallel(session).await,
            _ => Err(Error::Agent(
        ...
        if let Some(session_arc) = self.get_session(session_id).await {
            ...
            for agent_id in agent_ids {
                if let Some(state) = session.get_agent_state(&agent_id).await {
                    if matches!(
                        state,
                        AgentExecutionState::Running | AgentExecutionState::Pending
                    ) {
                        session
                            .update_agent_state(&agent_id, AgentExecutionState::Cancelled)
                            .await;
                    }
                }
            }
