...
mod tests {
    ...
    async fn test_error_handling() {
        ...
        let result = quick("agent", "You are an assistant").build();
    ...
    async fn test_convenience_model_creation() {
        ...
        let _anthropic_provider =
            anthropic("claude-3-sonnet").expect("Failed to create Anthropic provider");
        let _deepseek_provider =
            deepseek("deepseek-chat").expect("Failed to create DeepSeek provider");
