    ...
    pub fn update_request(&mut self, response_time_ms: f64, success: bool) {
        ...
        if self.total_requests == 1 {
            ...
            self.average_response_time_ms = 
                (self.average_response_time_ms * (self.total_requests - 1) as f64 + response_time_ms) 
...
impl AgentStateManager {
    ...
    /// 转换状态
    pub async fn transition_to(&self, new_state: AgentState, metadata: HashMap<String, serde_json::Value>) -> Result<()> {
        let mut current_state = self.current_state.write().await;
        let now = Utc::now();
        
        let duration = now.signed_duration_since(current_state.timestamp).num_seconds();
        ...
        *current_state = new_context.clone();
        ...
        // 保存到持久化存储
        self.persistence.save_state(&self.agent_id, &new_context).await?;
    ...
    pub async fn update_health_status(&self, health_status: HealthStatus) -> Result<()> {
        ...
        // 保存更新后的状态
        self.persistence.save_state(&self.agent_id, &*current_state).await?;
        
        Ok(())
    }
    
    /// 更新性能指标
    pub async fn update_performance_metrics(&self, update_fn: impl FnOnce(&mut PerformanceMetrics)) -> Result<()> {
        let mut current_state = self.current_state.write().await;
    ...
    pub async fn perform_health_check(&self) -> Result<HealthStatus> {
        ...
        // 确定健康状态
        let health_status = if error_rate > 0.5 { // 错误率超过50%
            HealthStatus::Unhealthy(format!("High error rate: {:.1}%", error_rate * 100.0))
        } else if !response_time_ok {
            HealthStatus::Warning(format!("High response time: {:.1}ms", metrics.average_response_time_ms))
    ...
    pub async fn reset(&self) -> Result<()> {
        self.transition_to(AgentState::Ready, HashMap::new()).await?;
        Ok(())
    }
    
    /// 停止Agent
    pub async fn shutdown(&self) -> Result<()> {
        self.transition_to(AgentState::ShuttingDown, HashMap::new()).await?;
        tokio::time::sleep(Duration::from_secs(1)).await;
        self.transition_to(AgentState::Stopped, HashMap::new()).await?;
        Ok(())
    }
}
...
pub trait AgentStateListener: Send + Sync {
    /// 当状态变更时被调用
    async fn on_state_changed(&self, old_state: &AgentStateContext, new_state: &AgentStateContext) -> Result<()>;
    
    /// 当健康状态变更时被调用
    async fn on_health_changed(&self, agent_id: &str, old_health: &HealthStatus, new_health: &HealthStatus) -> Result<()>;
}
...
mod tests {
    ...
    async fn test_agent_state_transitions() {
        ...
        // 转换到Ready状态
        manager.transition_to(AgentState::Ready, HashMap::new()).await.unwrap();
        let state = manager.get_current_state().await;
        ...
        // 转换到Running状态
        manager.transition_to(AgentState::Running, HashMap::new()).await.unwrap();
        assert!(manager.is_running().await);
    ...
    async fn test_state_persistence() {
        ...
        // 设置状态
        manager1.transition_to(AgentState::Running, HashMap::new()).await.unwrap();
