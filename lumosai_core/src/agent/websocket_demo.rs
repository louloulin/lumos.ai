...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Connections: {}",
        websocket_manager.connection_count().await
    );
    ...
    websocket_manager
        .add_connection("client_1".to_string(), "session_demo".to_string(), tx1)
        .await?;

    websocket_manager
        .add_connection("client_2".to_string(), "session_demo".to_string(), tx2)
        .await?;
    ...
    let messages = vec![user_message(
        "Hello! Can you demonstrate your WebSocket streaming capabilities?",
    )];
        ...
        while let Some(message) = rx1.recv().await {
            match message {
                WebSocketMessage::Connected {
                    ...
                    println!(
                        "  📱 Client 1 Connected: {} in session {} at {}",
                ...
                WebSocketMessage::SessionStart {
                    ...
                    println!(
                        "  🎬 Client 1 Session Started: {} at {}",
                        session_id, timestamp
                    );
                }
                WebSocketMessage::AgentEvent {
                    event,
                    session_id,
                    timestamp,
                } => {
                    println!(
                        "  🤖 Client 1 Agent Event in {}: {:?} at {}",
                        session_id, event, timestamp
                    );
                }
                WebSocketMessage::SessionEnd {
                    session_id,
                    timestamp,
                } => {
                    println!(
                        "  🏁 Client 1 Session Ended: {} at {}",
                        session_id, timestamp
                    );
                    break;
                }
        ...
        while let Some(message) = rx2.recv().await {
            match message {
                WebSocketMessage::Connected {
                    ...
                    println!(
                        "  📱 Client 2 Connected: {} in session {} at {}",
                        client_id, session_id, timestamp
                    );
                }
                WebSocketMessage::SessionStart {
    ...
    let mut stream = ws_agent
        .execute_with_websocket(&messages, &options, "session_demo")
        .await;
    ...
    while let Some(event_result) = stream.next().await {
        match event_result {
            Ok(event) => match &event {
                ...
                lumosai_core::agent::AgentEvent::GenerationComplete {
                    final_response,
                    total_steps,
                } => {
                    println!("\n\n✅ Generation Complete!");
