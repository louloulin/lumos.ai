...
impl ApiKeyManager {
    ...
    pub async fn generate_key(
        ...
        self.generate_key_with_expiration(user_id, name, scopes, None)
            .await
    }
    ...
    pub async fn generate_key_with_expiration(
        ...
        let expiration = expires_at.unwrap_or_else(|| SystemTime::now() + self.default_expiration);
    ...
    pub async fn list_user_keys(&self, user_id: &Uuid) -> Vec<&ApiKey> {
        if let Some(key_ids) = self.user_keys.get(user_id) {
            key_ids
                .iter()
                .filter_map(|key_id| self.keys.values().find(|key| key.id == *key_id))
                .collect()
        } else {
            Vec::new()
        }
...
mod tests {
    ...
    async fn test_list_user_keys() {
        ...
        manager
            .generate_key(user_id, "key1", vec!["agents:read".to_string()])
            .await
            .unwrap();
        manager
            .generate_key(user_id, "key2", vec!["tools:read".to_string()])
            .await
            .unwrap();
