...
async fn test_complete_auth_workflow() {
    ...
    let user = auth_manager
        .create_user(user_email, user_password, None)
        .await
        .unwrap();
    ...
    let validated_user = auth_manager
        .validate_token(&auth_token.token)
        .await
        .unwrap();
    ...
    let api_key = auth_manager
        .generate_api_key(user.id, "test-key", vec!["agents:read".to_string()])
        .await
        .unwrap();
    ...
    let has_permission = auth_manager
        .check_permission(&user.id, "agents:read")
        .await
        .unwrap();
...
async fn test_rbac_system() {
    ...
    // Test permission checking
    let has_create_permission = rbac
        .check_permission(&user_id, "agents:create")
        .await
        .unwrap();
    assert!(has_create_permission);

    let has_admin_permission = rbac
        .check_permission(&user_id, "admin:delete")
        .await
        .unwrap();
    ...
    let has_any_permission = rbac
        .check_permission(&user_id, "any:permission")
        .await
        .unwrap();
...
async fn test_api_key_management() {
    ...
    let api_key = api_manager
        .generate_key(user_id, "test-key", vec!["agents:read".to_string()])
        .await
        .unwrap();
    assert!(api_key.starts_with("lum_"));
...
async fn test_oauth2_flow() {
    ...
    let auth_url = oauth2_manager
        .start_authorization("google", None)
        .await
        .unwrap();
...
async fn test_multi_tenant_functionality() {
    ...
    let can_add_users = tenant_manager
        .check_resource_limit(&tenant.id, "users", 50)
        .await
        .unwrap();
    assert!(can_add_users);

    let cannot_add_too_many_users = tenant_manager
        .check_resource_limit(&tenant.id, "users", 200)
        .await
        .unwrap();
    ...
    // Test usage tracking
    tenant_manager
        .update_usage(&tenant.id, "users", 10)
        .await
        .unwrap();
    ...
    // Test domain setting
    tenant_manager
        .set_tenant_domain(&tenant.id, "testcompany.com".to_string())
        .await
        .unwrap();
