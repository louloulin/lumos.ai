...
impl Claims {
    ...
    pub fn to_user(&self) -> AuthResult<User> {
        ...
        let tenant_id =
            if let Some(tenant_str) = &self.tenant_id {
                Some(Uuid::parse_str(tenant_str).map_err(|_| {
                    AuthError::InvalidToken("Invalid tenant ID in token".to_string())
                })?)
            } else {
                None
            };
...
impl JWTManager {
    ...
    pub async fn generate_token(&self, user: &User) -> AuthResult<AuthToken> {
        ...
        let token =
            general_purpose::STANDARD.encode(format!("{}:{}", self.config.secret, token_data));
