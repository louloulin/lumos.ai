...
impl TenantManager {
    ...
    pub async fn create_tenant(
        ...
        self.usage_tracking
            .insert(tenant_id, ResourceUsage::default());
    ...
    pub async fn check_resource_limit(
        ...
        let usage = self.usage_tracking.get(tenant_id).unwrap_or(&default_usage);
        ...
        let can_proceed = match resource_type {
            ...
            "concurrent_executions" => {
                usage.concurrent_executions + requested_amount <= limits.max_concurrent_executions
            }
        ...
        match resource_type {
            ...
            "api_calls" => {
                usage.api_calls_this_month =
                    (usage.api_calls_this_month as i64 + delta as i64).max(0) as u64;
            }
            "concurrent_executions" => {
                usage.concurrent_executions =
                    (usage.concurrent_executions as i32 + delta).max(0) as u32;
            }
...
mod tests {
    ...
    async fn test_resource_limit_checking() {
        ...
        let tenant = manager
            .create_tenant("Test Tenant".to_string(), owner_id, SubscriptionPlan::Free)
            .await
            .unwrap();

        // Should be able to add users within limit
        let can_add_users = manager
            .check_resource_limit(&tenant.id, "users", 3)
            .await
            .unwrap();
        assert!(can_add_users);

        // Should not be able to add users beyond limit
        let cannot_add_users = manager
            .check_resource_limit(&tenant.id, "users", 10)
            .await
            .unwrap();
    ...
    async fn test_domain_mapping() {
        ...
        // Set domain
        manager
            .set_tenant_domain(&tenant.id, "example.com".to_string())
            .await
            .unwrap();
    ...
    async fn test_usage_tracking() {
        ...
        manager
            .update_usage(&tenant.id, "agents", 10)
            .await
            .unwrap();
