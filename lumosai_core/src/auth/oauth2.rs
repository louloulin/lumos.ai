...
impl OAuth2Manager {
    ...
    pub async fn start_authorization(
        ...
        let provider = self.providers.get(provider_name).ok_or_else(|| {
            AuthError::Other(format!("OAuth2 provider '{}' not found", provider_name))
        })?;
    ...
    pub async fn handle_callback(&mut self, code: &str, state: &str) -> AuthResult<AuthToken> {
        // Validate state
    ...
    fn create_user_from_oauth2(&self, user_info: &OAuth2UserInfo) -> AuthResult<User> {
        ...
        // Add OAuth2 metadata
        user.metadata
            .insert("oauth2_provider".to_string(), user_info.provider.clone());
        user.metadata
            .insert("oauth2_id".to_string(), user_info.id.clone());
