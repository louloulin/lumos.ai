...
impl SessionManager {
    ...
    pub async fn create_session(&mut self, user_id: Uuid) -> AuthResult<String> {
        self.create_session_with_timeout(user_id, None, self.default_timeout)
            .await
    }
    ...
    pub async fn create_session_with_timeout(
        ...
        let oldest_session_to_remove =
            if let Some(user_session_list) = self.user_sessions.get(&user_id) {
