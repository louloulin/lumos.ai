...
impl UsageRecord {
    /// 创建新的使用量记录
    pub fn new(tenant_id: Uuid, resource_type: String, quantity: u64, unit: String) -> Self {
        Self {
...
impl UsageTracker {
    ...
    pub fn get_usage_stats(
        ...
        // 缓存结果
        self.stats_cache
            .insert(cache_key, (stats.clone(), SystemTime::now()));
    ...
    fn calculate_usage_stats(
        ...
        for (resource_type, records) in usage_by_resource {
            ...
            let average_usage = if usage_count > 0 {
                total_usage as f64 / usage_count as f64
            } else {
                0.0
            };
            let peak_usage = records.iter().map(|r| r.quantity).max().unwrap_or(0);

            let resource_cost: f64 = records.iter().filter_map(|r| r.cost).sum();
            let average_cost = if usage_count > 0 {
                resource_cost / usage_count as f64
            } else {
                0.0
            };
    ...
    fn invalidate_cache(&mut self, tenant_id: &Uuid) {
        ...
        self.stats_cache
            .retain(|key, _| !key.starts_with(&tenant_str));
    }
