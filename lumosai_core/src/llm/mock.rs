...
impl MockLlmProvider {
    ...
    pub fn new_with_sequential_embeddings(
        start_value: f32,
        step: f32,
        dims: usize,
        count: usize,
...
impl LlmProvider for MockLlmProvider {
    ...
    async fn generate_stream<'a>(
        ...
        let stream = stream::iter(chunks).map(Ok).boxed();
...
mod tests {
    ...
    async fn test_mock_llm_embedding() {
        // 初始化时提供两个嵌入向量
        let mock =
            MockLlmProvider::new_with_embeddings(vec![vec![0.1, 0.2, 0.3], vec![0.4, 0.5, 0.6]]);
