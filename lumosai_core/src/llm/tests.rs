...
mod tests {
    ...
    async fn test_anthropic_embedding_error() {
        ...
        let provider = AnthropicProvider::new("fake-api-key".to_string(), "claude-2".to_string());
    ...
    async fn test_openai_integration() {
        ...
        let provider = OpenAiProvider::new(api_key, "gpt-3.5-turbo".to_string());
    ...
    async fn test_anthropic_integration() {
        // 只有在提供API密钥时才运行此测试
        let api_key =
            std::env::var("ANTHROPIC_API_KEY").expect("需要设置ANTHROPIC_API_KEY环境变量");

        let provider = AnthropicProvider::new(api_key, "claude-3-opus-20240229".to_string());
    ...
    async fn test_deepseek_integration_with_messages() {
        ...
        let provider = DeepSeekProvider::new(api_key, Some("deepseek-chat".to_string()));
