...
impl SemanticMemory {
    ...
    async fn init_vector_storage(&self) -> Result<()> {
        ...
        if !indexes.contains(&"default".to_string()) {
            self.vector_storage
                .create_index("default", 1536, None)
                .await?;
        }
        Ok(())
    }
    ...
    pub async fn get_stats(&self) -> Result<HashMap<String, serde_json::Value>> {
        ...
        if let Ok(index_stats) = self.vector_storage.describe_index("default").await {
            stats.insert(
                ...
                serde_json::json!(index_stats.dimension),
            );
            stats.insert(
                "vector_count".to_string(),
                serde_json::json!(index_stats.count),
            );
            stats.insert(
                "similarity_metric".to_string(),
                serde_json::json!(format!("{:?}", index_stats.metric)),
            );
        }
...
impl Memory for SemanticMemory {
    ...
    async fn retrieve(&self, config: &MemoryConfig) -> Result<Vec<Message>> {
        ...
        if let Some(semantic_config) = &config.semantic_recall {
            ...
            // 收集消息ID
            let message_ids: Vec<String> = results.iter().map(|result| result.id.clone()).collect();
...
mod tests {
    ...
    async fn test_semantic_memory_store() {
        ...
        // 创建语义内存
        let semantic_memory =
            SemanticMemory::new(&config, Arc::clone(&mock_llm) as Arc<dyn LlmProvider>).unwrap();
    ...
    async fn test_semantic_memory_retrieve() {
        ...
        // 验证检索结果内容
        let contains_message1 = messages
            .iter()
            .any(|msg| msg.role == Role::User && msg.content.contains("语义搜索"));
        assert!(contains_message1, "检索结果应包含用户询问的消息");

        let contains_message2 = messages
            .iter()
            .any(|msg| msg.role == Role::Assistant && msg.content.contains("语义理解"));
    ...
    async fn test_semantic_memory_stats() {
        ...
        assert_eq!(
            stats.get("namespace").and_then(|v| v.as_str()),
            Some("test_stats")
        );
        assert_eq!(stats.get("message_count").and_then(|v| v.as_u64()), Some(1));
        assert_eq!(
            stats.get("vector_dimension").and_then(|v| v.as_u64()),
            Some(1536)
        );
