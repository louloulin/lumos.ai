...
impl EncryptionManager {
    ...
    pub async fn new(config: &EncryptionConfig) -> Result<Self> {
        ...
        let unbound_key = match config.algorithm {
            EncryptionAlgorithm::AES256GCM => {
                aead::UnboundKey::new(&aead::AES_256_GCM, &master_key).map_err(|_| {
                    LumosError::SecurityError("Failed to create AES key".to_string())
                })?
            }
            EncryptionAlgorithm::ChaCha20Poly1305 => {
                aead::UnboundKey::new(&aead::CHACHA20_POLY1305, &master_key).map_err(|_| {
                    LumosError::SecurityError("Failed to create ChaCha20 key".to_string())
                })?
            }
        };
    ...
    pub async fn encrypt(&self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        let serialized = serde_json::to_vec(&encrypted_data).map_err(|e| {
            LumosError::SecurityError(format!("Failed to serialize encrypted data: {}", e))
        })?;
        Ok(serialized)
    }
    ...
    pub async fn decrypt(&self, encrypted_data: &[u8]) -> Result<Vec<u8>> {
        let encrypted_data: EncryptedData =
            serde_json::from_slice(encrypted_data).map_err(|e| {
                LumosError::SecurityError(format!("Failed to deserialize encrypted data: {}", e))
            })?;
    ...
    pub async fn decrypt_with_metadata(&self, encrypted_data: &EncryptedData) -> Result<Vec<u8>> {
        ...
        let mut ciphertext = general_purpose::STANDARD
            ...
            .map_err(|_| {
                LumosError::SecurityError("Failed to decode encrypted data".to_string())
            })?;
    ...
    pub async fn rotate_key(&mut self) -> Result<()> {
        ...
        self.rng.fill(&mut new_master_key).map_err(|_| {
            LumosError::SecurityError("Failed to generate new master key".to_string())
        })?;

        // 创建新的AEAD密钥
        let unbound_key = match self.config.algorithm {
            EncryptionAlgorithm::AES256GCM => {
                aead::UnboundKey::new(&aead::AES_256_GCM, &new_master_key).map_err(|_| {
                    LumosError::SecurityError("Failed to create new AES key".to_string())
                })?
            }
            EncryptionAlgorithm::ChaCha20Poly1305 => {
                aead::UnboundKey::new(&aead::CHACHA20_POLY1305, &new_master_key).map_err(|_| {
                    LumosError::SecurityError("Failed to create new ChaCha20 key".to_string())
                })?
            }
        };
    ...
    async fn cleanup_expired_keys(&mut self) -> Result<()> {
        let cutoff =
            Utc::now() - chrono::Duration::hours(self.config.key_rotation_hours as i64 * 7); // 保留7个轮换周期
