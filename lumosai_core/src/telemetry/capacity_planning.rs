...
impl CapacityPlanner {
    ...
    ) -> Result<Vec<PredictionResult>, LumosError> {
        self.prediction_engine
            .predict_capacity(resource_type, &self.config)
            .await
    }

    /// 生成扩容建议
    pub async fn generate_scaling_recommendations(
        &self,
    ) -> Result<Vec<ScalingRecommendation>, LumosError> {
    ...
    pub async fn generate_capacity_report(&self) -> Result<CapacityPlanningReport, LumosError> {
        ...
        let cost_analysis = self
            .cost_analyzer
            .analyze_costs(&resource_forecasts)
            .await?;
    ...
    fn generate_summary(
        &self,
        forecasts: &HashMap<ResourceType, Vec<PredictionResult>>,
        ...
        format!(
            ...
            total_resources, critical_recommendations
        )
    }
}
...
impl CostAnalyzer {
    ...
    async fn analyze_costs(
        &self,
        _forecasts: &HashMap<ResourceType, Vec<PredictionResult>>,
    ) -> Result<CostAnalysisResult, LumosError> {
        // 简化实现
