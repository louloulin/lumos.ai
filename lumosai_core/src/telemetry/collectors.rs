...
impl MetricsCollector for InMemoryMetricsCollector {
    ...
    async fn get_metrics_summary(
        ...
        // 过滤指标
        let filtered_metrics: Vec<&AgentMetrics> = metrics
            .iter()
            .filter(|m| {
        ...
        let execution_times: Vec<u64> = filtered_metrics
            .iter()
        ...
        let total_tokens_used: u64 = filtered_metrics
            .iter()
        ...
        for metric in &filtered_metrics {
            *tool_call_stats
                .entry("total_calls".to_string())
                .or_insert(0) += metric.tool_calls_count as u64;
        }
    ...
    async fn get_agent_performance(
        ...
        // 过滤指定代理的指标
        let agent_metrics: Vec<&AgentMetrics> = metrics
            .iter()
        ...
        let recent_metrics: Vec<&AgentMetrics> = agent_metrics
            .iter()
        ...
        for tool_metric in tool_metrics.iter() {
            *tool_usage
                .entry(tool_metric.tool_name.clone())
                .or_insert(0u64) += 1;
        }
...
impl TraceCollector for InMemoryMetricsCollector {
    ...
    async fn search_traces(
        ...
        let filtered_traces: Vec<ExecutionTrace> = traces
            .values()
            .filter(|trace| {
    ...
    async fn get_trace_stats(
        ...
        let mut durations: Vec<u64> = filtered_traces
            .iter()
        ...
        // 获取最慢的追踪
        let mut slowest_traces: Vec<(String, u64)> = filtered_traces
            .iter()
...
impl FileSystemMetricsCollector {
    ...
    /// 保存指标到文件
    async fn save_to_file<T: serde::Serialize>(
        &self,
        filename: &str,
        data: &T,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    /// 从文件加载指标
    async fn load_from_file<T: serde::de::DeserializeOwned>(
        &self,
        filename: &str,
...
impl MetricsCollector for FileSystemMetricsCollector {
    ...
    async fn record_tool_execution(
        ...
        let filename = format!(
            "tool_metrics_{}_{}.json",
            metrics.tool_name, metrics.timestamp
        );
        self.save_to_file(&filename, &metrics).await?;
    ...
    async fn record_memory_operation(
        ...
        let filename = format!(
            ...
            metrics.operation_type, metrics.timestamp
        );
        self.save_to_file(&filename, &metrics).await?;
...
impl TraceCollector for FileSystemMetricsCollector {
    ...
    async fn get_trace_stats(
        ...
        self.cache
            .get_trace_stats(agent_id, from_time, to_time)
            .await
    }
}
