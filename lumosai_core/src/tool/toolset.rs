...
use super::enhanced::{EnhancedTool, ToolCapability, ToolCategory};
...
impl ToolSet {
    ...
    pub fn stats(&self) -> ToolSetStats {
        ...
        for tool in self.tools.values() {
            ...
            match tool.category() {
                ToolCategory::General => stats.categories.insert(
                    "general".to_string(),
                    stats.categories.get("general").unwrap_or(&0) + 1,
                ),
                ToolCategory::Web => stats.categories.insert(
                    "web".to_string(),
                    stats.categories.get("web").unwrap_or(&0) + 1,
                ),
                ToolCategory::FileSystem => stats.categories.insert(
                    "filesystem".to_string(),
                    stats.categories.get("filesystem").unwrap_or(&0) + 1,
                ),
                ToolCategory::Database => stats.categories.insert(
                    "database".to_string(),
                    stats.categories.get("database").unwrap_or(&0) + 1,
                ),
                ToolCategory::AI => stats.categories.insert(
                    "ai".to_string(),
                    stats.categories.get("ai").unwrap_or(&0) + 1,
                ),
                ToolCategory::Communication => stats.categories.insert(
                    "communication".to_string(),
                    stats.categories.get("communication").unwrap_or(&0) + 1,
                ),
                ToolCategory::DataProcessing => stats.categories.insert(
                    "dataprocessing".to_string(),
                    stats.categories.get("dataprocessing").unwrap_or(&0) + 1,
                ),
                ToolCategory::System => stats.categories.insert(
                    "system".to_string(),
                    stats.categories.get("system").unwrap_or(&0) + 1,
                ),
                ToolCategory::Math => stats.categories.insert(
                    "math".to_string(),
                    stats.categories.get("math").unwrap_or(&0) + 1,
                ),
                ToolCategory::Custom(name) => stats
                    .categories
                    .insert(name.clone(), stats.categories.get(&name).unwrap_or(&0) + 1),
            };

            for capability in tool.capabilities() {
                match capability {
                    ToolCapability::Basic => stats.capabilities.insert(
                        "basic".to_string(),
                        stats.capabilities.get("basic").unwrap_or(&0) + 1,
                    ),
                    ToolCapability::Streaming => stats.capabilities.insert(
                        "streaming".to_string(),
                        stats.capabilities.get("streaming").unwrap_or(&0) + 1,
                    ),
                    ToolCapability::Async => stats.capabilities.insert(
                        "async".to_string(),
                        stats.capabilities.get("async").unwrap_or(&0) + 1,
                    ),
                    ToolCapability::Batch => stats.capabilities.insert(
                        "batch".to_string(),
                        stats.capabilities.get("batch").unwrap_or(&0) + 1,
                    ),
                    ToolCapability::Caching => stats.capabilities.insert(
                        "caching".to_string(),
                        stats.capabilities.get("caching").unwrap_or(&0) + 1,
                    ),
                    ToolCapability::RateLimit => stats.capabilities.insert(
                        "ratelimit".to_string(),
                        stats.capabilities.get("ratelimit").unwrap_or(&0) + 1,
                    ),
                    ToolCapability::Auth => stats.capabilities.insert(
                        "auth".to_string(),
                        stats.capabilities.get("auth").unwrap_or(&0) + 1,
                    ),
                    ToolCapability::Encryption => stats.capabilities.insert(
                        "encryption".to_string(),
                        stats.capabilities.get("encryption").unwrap_or(&0) + 1,
                    ),
                    ToolCapability::Monitoring => stats.capabilities.insert(
                        "monitoring".to_string(),
                        stats.capabilities.get("monitoring").unwrap_or(&0) + 1,
                    ),
                    ToolCapability::Custom(name) => stats.capabilities.insert(
                        name.clone(),
                        stats.capabilities.get(&name).unwrap_or(&0) + 1,
                    ),
                };
            }
        }
