...
use super::{FilterCondition, IndexStats, QueryResult, SimilarityMetric, VectorStorage};
...
impl VectorStorage for SqliteVectorStorage {
    async fn create_index(
        ...
        let conn = self
            .conn
            .lock()
            .map_err(|_| Error::Storage("Failed to acquire lock".into()))?;
    ...
    async fn list_indexes(&self) -> Result<Vec<String>> {
        ...
        let mut stmt = conn
            .prepare("SELECT name FROM indexes")
    ...
    async fn delete_index(&self, index_name: &str) -> Result<()> {
        ...
        tx.execute(
            "DELETE FROM vectors WHERE index_name = ?",
            params![index_name],
        )
        ...
        tx.execute(
            "DELETE FROM metadata WHERE index_name = ?",
            params![index_name],
        )
    ...
    async fn query(
        ...
        let result: Result<(i64, String, i64)> = conn
            .query_row(
                "SELECT dimension, metric, count FROM indexes WHERE name = ?",
                params![index_name],
                |row| {
        ...
        for row in rows {
            let (id, vector_json, meta_json) =
                row.map_err(|e| Error::Storage(format!("Failed to get row: {}", e)))?;
            ...
            let metadata: Option<HashMap<String, Value>> = if let Some(json) = meta_json {
                Some(serde_json::from_str(&json).map_err(|e| {
                    Error::Storage(format!("Failed to deserialize metadata: {}", e))
                })?)
            } else {
                None
            };
...
mod tests {
    ...
    async fn test_sqlite_vector_operations() {
        ...
        let ids = storage
            .upsert("test_index", vectors, None, metadata)
            .await
            .unwrap();
        assert_eq!(ids.len(), 2);
        ...
        // Update vector
        storage
            .update_by_id("test_index", &ids[0], Some(vec![0.0, 0.0, 1.0]), None)
            .await
            .unwrap();
