...
use crate::logger::{Component, LogLevel, Logger};
use crate::telemetry::TelemetrySink;
use crate::voice::{ListenOptions, VoiceListener, VoiceOptions, VoiceProvider, VoiceSender};
    ...
    pub fn new(config: OpenAIVoiceConfig) -> Result<Self> {
        if config.api_key.is_none() {
            return Err(Error::Configuration(
                "OpenAI API密钥未设置，请设置OPENAI_API_KEY环境变量或在配置中提供".to_string(),
            ));
        }
...
impl VoiceProvider for OpenAIVoice {
    ...
    async fn speak(
        ...
        self.logger()
            .debug(&format!("OpenAIVoice: 将文本转换为语音: {}", text), None);

        let voice_id = options
            .voice_id
            .as_deref()
            .unwrap_or_else(|| self.config.default_voice.as_deref().unwrap_or("alloy"));
    ...
    async fn listen(&self, _audio: Vec<u8>, _options: &ListenOptions) -> Result<String> {
        ...
        Err(Error::Unsupported(
            "OpenAIVoice STT功能尚未实现".to_string(),
        ))
    }
    ...
    async fn send(&self, _audio: Vec<u8>) -> Result<()> {
        // OpenAI目前不支持实时语音流
        Err(Error::Unsupported(
            "OpenAIVoice不支持实时语音流功能".to_string(),
        ))
    }
