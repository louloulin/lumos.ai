use super::types::{RetryConfig, Step, StepContext};
...
pub struct BasicStep {
    ...
    /// 步骤执行函数
    execute_fn: Arc<
        dyn Fn(StepContext) -> futures::future::BoxFuture<'static, Result<serde_json::Value, Error>>
            + Send
            + Sync,
    >,
}
...
impl BasicStep {
    ...
    where
        F: Fn(serde_json::Value) -> Result<serde_json::Value, Error>
            + Send
            + Sync
            + 'static
            + Clone,
...
impl StepBuilder {
    ...
    where
        ...
        Fut: futures::Future<Output = Result<serde_json::Value, Error>> + Send + 'static,
    {
        BasicStep::new(self.id, self.description, execute_fn, self.retry_config)
    }
}
