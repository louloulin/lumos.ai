...
mod tests {
    ...
    impl Agent for MockAgent {
        async fn generate(
            &self,
            messages: &[Message],
            _options: &crate::agent::types::AgentGenerateOptions,
    ...
    async fn test_basic_workflow() {
        ...
        assert!(
            output.is_object() || output.is_string(),
            "输出应该是对象或字符串"
        );

        let output_str = output.to_string();
        assert!(
            output_str.contains("已处理:") || output_str.contains("Processed:"),
            "输出应包含处理后的内容"
        );
    }
    ...
    async fn test_workflow_error_handling() {
        // 创建会失败的模拟代理
        let failing_agent = MockAgent::new(|_| Err(Error::Agent("模拟代理故意失败".to_string())));
        ...
        if let Some(failed_step) = output_map.get("失败步骤") {
            ...
            assert!(
                output.to_string().contains("模拟代理故意失败"),
                "输出应该包含失败步骤的错误信息"
            );
        }
        ...
        if let Some(final_step) = output_map.get("最终步骤") {
            ...
            // 输出本身可能就是最终步骤的结果
            assert!(
                output.to_string().contains("这个步骤应该总是执行"),
                "输出应该包含最终步骤的处理结果"
            );
        }
        ...
        assert!(
            !output.to_string().contains("这个步骤应该不会执行"),
            "条件步骤不应该被执行"
        );
    }
}
