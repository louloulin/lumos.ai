...
use super::types::{
    ActivePathInfo, DependencyCheckOutput, RetryConfig, Step, StepCondition, StepContext,
    StepExecutorOutput, StepResult, StepStatus, StepStatusInfo, WorkflowRunResult, WorkflowState,
};
    ...
    async fn run_step(&self, step_id: &str, node: &StepNode) -> Result<StepExecutorOutput, Error> {
        ...
        match node.step.execute(context).await {
            Ok(output) => {
                ...
                state
                    .active_paths
                    .retain(|p| p.step_path != vec![step_id.to_string()]);
            ...
            Err(err) => {
                ...
                Ok(StepExecutorOutput::StepFailed {
                    error: err.to_string(),
                })
            }
        }
...
impl WorkflowBuilder {
    ...
    pub fn add_step<S: Step + 'static>(
        ...
        if let Some(data) = input_data {
            ...
            self.step_data
                .insert(step_id.clone(), serde_json::json!({}));
        }
