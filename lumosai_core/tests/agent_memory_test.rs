...
mod tests {
    ...
    impl MemoryThreadStorage for MockMemoryThreadStorage {
        ...
        async fn delete_messages(
            ...
            thread_id: &str,
            _message_ids: &[String],
        ) -> Result<lumosai_core::memory::thread::MessageOperationResult> {
        ...
        async fn search_messages(
            &self,
            query: &str,
            _filter: Option<&lumosai_core::memory::thread::MessageFilter>,
        ) -> Result<Vec<Message>> {
            let messages = self.messages.lock().unwrap();
        ...
        async fn get_thread_stats(
            &self,
            thread_id: &str,
        ) -> Result<lumosai_core::memory::thread::ThreadStats> {
            let threads = self.threads.lock().unwrap();
            let messages = self.messages.lock().unwrap();

            let thread = threads.get(thread_id).ok_or_else(|| {
                lumosai_core::error::Error::NotFound(format!("Thread {} not found", thread_id))
            })?;

            let empty_vec = Vec::new();
            let thread_messages = messages.get(thread_id).unwrap_or(&empty_vec);

            let user_count = thread_messages
                .iter()
                .filter(|m| m.role == Role::User)
                .count();
            let assistant_count = thread_messages
                .iter()
    ...
    async fn test_generate_with_memory_basic() -> Result<()> {
        ...
        let result = agent
            .generate_with_memory(&messages, Some("test_thread_123".to_string()), &options)
            .await;
    ...
    async fn test_generate_with_memory_conversation_history() -> Result<()> {
        ...
        // First message
        let first_message = vec![Message::new(
            Role::User,
        ...
        let second_result = agent
            .generate_with_memory(&second_message, Some(thread_id), &options)
            .await?;
        assert_eq!(
    ...
    async fn test_generate_with_memory_without_memory_storage() -> Result<()> {
        ...
        let result = agent
            .generate_with_memory(&messages, Some("test_thread".to_string()), &options)
            .await;
        assert!(result.is_ok());
        let response = result?;
    ...
    async fn test_memory_thread_message_storage() -> Result<()> {
        ...
        // Add messages
        let user_message = Message::new(Role::User, "Hello, AI!".to_string(), None, None);
        ...
        memory_storage
            .add_message(thread_id, &assistant_message)
            .await?;
    ...
    async fn generate_with_memory_persists_thread_metadata() -> Result<()> {
        ...
        let agent = BasicAgent::new(config, llm).with_memory(memory.clone() as Arc<dyn Memory>);
    ...
    async fn test_agent_thread_management_through_memory() -> Result<()> {
        use lumosai_core::memory::thread::{
            CreateThreadParams, InMemoryThreadStorage, UpdateThreadParams,
        };

        let llm = create_test_zhipu_provider_arc();
        let storage = Arc::new(InMemoryThreadStorage::default());
        let memory = Arc::new(lumosai_core::memory::BasicMemory::with_thread_storage(
            None,
            None,
            Some(storage.clone()),
        )) as Arc<dyn Memory>;
        ...
        // 通过 Memory 获取统计信息
        let stats = memory
            .get_thread_stats("agent-thread", Some("user-123"))
            .await?;
        assert_eq!(stats.message_count, 0);

        // 通过 Memory 删除线程
        memory
            .delete_thread("agent-thread", Some("user-123"))
            .await?;
    ...
    async fn test_memory_agent_trait_default_implementation() -> Result<()> {
        ...
        impl lumosai_core::base::Base for TestMemoryAgent {
            ...
            fn telemetry(
                &self,
            ) -> Option<std::sync::Arc<dyn lumosai_core::telemetry::TelemetrySink>> {
                None
            }

            fn set_telemetry(
                &mut self,
                _telemetry: std::sync::Arc<dyn lumosai_core::telemetry::TelemetrySink>,
            ) {
            }
        }
        ...
        let new_message = Message::new(Role::User, "What's my name?".to_string(), None, None);
        let options = AgentGenerateOptions {
    ...
    async fn test_memory_trait_add_processor_and_process_messages() -> Result<()> {
        ...
        let memory: Arc<dyn Memory> =
            Arc::new(lumosai_core::memory::BasicMemory::with_thread_storage(
                None,
                None,
                Some(storage.clone()),
            ));
        ...
        // 创建一些消息
        let messages = (0..100)
            .map(|i| Message::new(Role::User, format!("Message {}", i), None, None))
            .collect::<Vec<_>>();
