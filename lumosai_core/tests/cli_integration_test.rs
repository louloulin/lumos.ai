...
use lumosai_core::cli::{
    commands::Commands,
    enhanced_errors::{ErrorHandler, LumosCliError},
    web_interface::{CreateSessionRequest, SendMessageRequest, WebInterface, WebInterfaceConfig},
    CliUtils, ProjectConfig,
};
...
async fn test_project_creation() {
    ...
    assert!(
        project_path.join("lumos.toml").exists(),
        "lumos.toml should exist"
    );
...
async fn test_web_agent_template() {
    ...
    assert!(
        config.tools.iter().any(|t| t.name == "web_search"),
        "Should have web_search tool"
    );
    assert!(
        config.tools.iter().any(|t| t.name == "http_request"),
        "Should have http_request tool"
    );

    // Verify main.rs contains web-specific content
    let main_rs_content =
        std::fs::read_to_string(project_path.join("src/main.rs")).expect("Should read main.rs");
    assert!(
        main_rs_content.contains("web-savvy assistant"),
        "Should contain web-specific instructions"
    );
    assert!(
        main_rs_content.contains("web_search"),
        "Should reference web tools"
    );
}
...
async fn test_data_agent_template() {
    ...
    assert!(!config.tools.is_empty(), "Should have tools configured");
    assert!(
        config.tools.iter().any(|t| t.name == "csv_reader"),
        "Should have csv_reader tool"
    );
    assert!(
        config.tools.iter().any(|t| t.name == "data_analyzer"),
        "Should have data_analyzer tool"
    );
...
async fn test_chatbot_template() {
    ...
    assert!(!config.tools.is_empty(), "Should have tools configured");
    assert!(
        config.tools.iter().any(|t| t.name == "memory"),
        "Should have memory tool"
    );
    assert!(
        config.tools.iter().any(|t| t.name == "calculator"),
    ...
    let main_rs_content =
        std::fs::read_to_string(project_path.join("src/main.rs")).expect("Should read main.rs");
    assert!(
        main_rs_content.contains("Interactive Chatbot"),
        "Should contain chatbot title"
    );
    assert!(
        main_rs_content.contains("loop"),
        "Should contain interaction loop"
    );
}
...
async fn test_cli_utils() {
    ...
    assert!(
        !CliUtils::is_lumos_project(&temp_dir.path()),
        "Should not detect project initially"
    );
    ...
    assert!(
        CliUtils::is_lumos_project(&temp_dir.path()),
        "Should detect project after config creation"
    );
...
async fn test_web_interface() {
    ...
    let message_response = web_interface
        .send_message(&session.session_id, message_request)
        .await;
...
async fn test_enhanced_error_handling() {
    ...
    assert!(
        !enhanced.related_docs.is_empty(),
        "Should have related docs"
    );
    ...
    assert!(
        enhanced
            .related_docs
            .iter()
            .any(|doc| doc.contains("tools")),
        "Should have tool-related docs"
    );
}
...
async fn test_template_validation() {
    ...
    for template in templates {
        ...
        assert!(
            project_path.join("lumos.toml").exists(),
            "Config should exist for template '{}'",
            template
        );
    }
...
async fn test_error_message_quality() {
    ...
    assert!(
        error_msg.contains("lumos tools add"),
        "Should provide fix command"
    );
    ...
    assert!(
        error_msg.contains("internet connection"),
...
async fn test_cli_performance() {
    ...
    assert!(
        duration.as_secs() < 5,
        "Should create 5 projects in under 5 seconds, took {:?}",
        duration
    );
}
