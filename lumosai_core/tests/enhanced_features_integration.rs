...
use lumosai_core::debug::{DebugEvent, DebugManager, DebugSession};
use lumosai_core::error::friendly::{helpers, FriendlyError};
use lumosai_core::error::Error;
use lumosai_core::logging::{LogFormat, LogLevel, Logger, LoggerConfig};
...
async fn test_friendly_error_helpers() {
    ...
    let config_error =
        helpers::config_error("Missing field 'api_key'", Some("/path/to/config.json"));
...
async fn test_logging_system() {
    ...
    let logger_with_corr = Logger::new("corr_module").with_correlation_id("req-123");
...
async fn test_marketplace_system() {
    ...
    let mut marketplace = Marketplace::new("https://registry.lumos.ai".to_string(), cache_dir);
...
async fn test_comprehensive_workflow() {
    ...
    let logger = Logger::new("comprehensive_test").with_correlation_id(session_id.clone());
    ...
    logger.log_with_fields(
        ...
        [
            ...
            (
                ...
                json!(search_results.as_ref().map(|r| r.len()).unwrap_or(0)),
            ),
        ]
        .into_iter()
        .collect(),
    ...
    logger.log_with_fields(
        ...
        [
            ...
            (
                ...
                json!(summary.total_duration.as_millis()),
            ),
            (
                "tool_executions".to_string(),
                json!(summary.tool_executions),
            ),
...
async fn test_performance_and_scalability() {
    ...
    for i in 0..100 {
        let logger =
            Logger::new(&format!("module_{}", i)).with_correlation_id(format!("corr_{}", i));
        loggers.push(logger);
    }
    ...
    for (i, logger) in loggers.iter().enumerate() {
        logger.log_with_fields(
            ...
            [("index".to_string(), json!(i))].into_iter().collect(),
        );
    }
