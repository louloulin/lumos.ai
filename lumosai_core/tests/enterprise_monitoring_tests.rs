...
use lumosai_core::{
    ...
    telemetry::{
        ...
        enterprise::*, sla_monitoring::*, AgentMetrics, AlertEvent, AlertManager, ExecutionTrace,
...
async fn test_compliance_monitoring() {
    ...
    assert!(
        report.compliance_score >= 0.0 && report.compliance_score <= 100.0,
        "合规分数应该在0-100之间"
    );
}
...
async fn test_business_metrics_collection() {
    ...
    let report = report.unwrap();
    assert!(
        !report.key_insights.is_empty() || report.key_insights.is_empty(),
        "报告应该包含洞察或为空"
    );
}
...
async fn test_capacity_planning() {
    ...
    assert!(
        !forecast.is_empty() || forecast.is_empty(),
        "预测结果应该有效"
    );
    ...
    let report = report.unwrap();
    assert!(
        report.cost_analysis.current_monthly_cost >= 0.0,
        "当前成本应该大于等于0"
    );
}
...
async fn test_sla_monitoring() {
    ...
    let report = report.unwrap();
    assert!(
        report.sla_summary.overall_compliance_rate >= 0.0,
        "合规率应该大于等于0"
    );
...
async fn test_configuration_validation() {
    ...
    assert!(
        !compliance_config.enabled_standards.is_empty(),
        "默认应该有启用的合规标准"
    );
    assert!(
        compliance_config.real_time_monitoring,
        "默认应该启用实时监控"
    );
    ...
    assert!(
        business_config.revenue_metrics_enabled,
        "默认应该启用收入指标"
    );
    assert!(
        business_config.usage_metrics_enabled,
        "默认应该启用使用指标"
    );
    ...
    assert!(
        anomaly_config.statistical_detection_enabled,
        "默认应该启用统计检测"
    );
    assert!(
        anomaly_config.behavior_detection_enabled,
        "默认应该启用行为检测"
    );
    ...
    assert!(
        capacity_config.prediction_window_days > 0,
        "预测窗口应该大于0"
    );
    assert!(
        capacity_config.auto_scaling_recommendations,
        "默认应该启用自动扩容建议"
    );
    ...
    assert!(
        sla_config.monitoring_interval_seconds > 0,
        "监控间隔应该大于0"
    );
}
...
async fn test_error_handling() {
    ...
    // 测试空数据的异常检测
    let result = detector
        .detect_metric_anomaly("empty_metric", 0.0, Utc::now())
        .await;
