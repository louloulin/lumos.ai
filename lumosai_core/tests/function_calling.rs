...
use lumosai_core::agent::{message_utils::user_message, AgentConfig, BasicAgent};
use lumosai_core::error::Result;
use lumosai_core::llm::mock::MockLlmProvider;
use lumosai_core::tool::{GenericTool, ParameterSchema, ToolSchema};
...
mod tests {
    ...
    async fn test_enhanced_tool_parsing() -> Result<()> {
        ...
        let result1 = agent
            .generate(&messages1, &AgentGenerateOptions::default())
            .await?;
        ...
        let result3 = agent
            .generate(&messages3, &AgentGenerateOptions::default())
            .await?;
    ...
    async fn test_edge_case_mixed_mode() -> Result<()> {
        ...
        // Create weather tool
        let weather_schema = ToolSchema::new(vec![ParameterSchema {
