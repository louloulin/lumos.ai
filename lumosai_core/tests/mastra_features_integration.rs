...
async fn test_mastra_compatible_api() {
    ...
    assert_eq!(
        ...
        "You are a Mastra-compatible assistant"
    );
}
...
async fn test_safe_vs_dev_tools() {
    ...
    let safe_tool_names: Vec<String> = safe_tools
        .iter()
        .filter_map(|t| t.name().map(|n| n.to_string()))
        .collect();
    ...
    let dev_tool_names: Vec<String> = dev_tools
        .iter()
        .filter_map(|t| t.name().map(|n| n.to_string()))
        .collect();
...
async fn test_mastra_utility_functions() {
    ...
    let quick_agent =
        mastra_compat::utils::quick_agent("quick_test", "Quick test agent", llm.clone())
    ...
    // Test web agent
    let web_agent = mastra_compat::utils::web_agent("web_test", "Web-enabled agent", llm.clone())
    ...
    // Test file agent
    let file_agent = mastra_compat::utils::file_agent("file_test", "File processing agent", llm)
...
async fn test_agent_config_creation() {
    let llm = Arc::new(MockLlmProvider::new(
        vec!["Config-based agent!".to_string()],
    ));
    ...
    let agent =
        mastra_compat::Agent::from_config(config).expect("Failed to create agent from config");
