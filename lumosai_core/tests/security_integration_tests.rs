...
async fn test_security_framework_integration() {
    ...
    match decision {
        ...
        AccessDecision::Conditional { conditions } => {
            println!("Conditional access: {:?}", conditions)
        }
    }

    // 测试威胁检测
    let threats = security
        .detect_threats(&access_request.context)
        .await
        .unwrap();
    ...
    // 测试合规性检查
    let compliance_report = security
        .check_compliance(ComplianceStandard::SOC2)
        .await
        .unwrap();
...
async fn test_audit_logger() {
    ...
    let report = logger
        .generate_compliance_report(start_time, end_time, "SOC2")
        .await
        .unwrap();
...
async fn test_compliance_monitor() {
    ...
    // 测试GDPR合规检查
    let gdpr_report = monitor
        .check_compliance(ComplianceStandard::GDPR)
        .await
        .unwrap();
...
async fn test_network_security_manager() {
    ...
    manager
        .block_ip(malicious_ip, Some(chrono::Duration::hours(24)))
        .await
        .unwrap();
...
async fn test_end_to_end_security_scenario() {
    ...
    security
        .log_security_event(data_access_event)
        .await
        .unwrap();
