...
impl BillingEngine {
    ...
    pub fn calculate_usage_cost(
        ...
        for (resource_type, resource_stats) in &usage_stats.usage_by_resource {
            // 获取定价规则
            let pricing_rule = self.pricing_rules.get(resource_type).ok_or_else(|| {
                BillingError::InvalidConfiguration(format!(
                    "No pricing rule found for resource: {}",
                    resource_type
                ))
            })?;
            ...
            // 应用折扣
            let discounted_cost =
                self.apply_discounts(tenant_id, resource_type, base_cost, subscription)?;
    ...
    fn calculate_resource_cost(
        ...
        match &pricing_rule.pricing_model {
            ...
            PricingModel::Hybrid {
                ...
                if let Some(rate) = usage_rates.get(&resource_stats.resource_type) {
                    let included = included_usage
                        .get(&resource_stats.resource_type)
                        .unwrap_or(&0);
    ...
    fn apply_subscription_discount(
        ...
        match discount.discount_type {
            DiscountType::Percentage => Ok(cost * (1.0 - discount.value / 100.0)),
            DiscountType::FixedAmount => Ok((cost - discount.value).max(0.0)),
    ...
    pub fn get_billing_history(&self, tenant_id: &Uuid) -> Vec<&Invoice> {
        self.billing_history
            .get(tenant_id)
            .map(|invoices| invoices.iter().collect())
            .unwrap_or_default()
    }
        ...
        for (resource_type, usage) in resource_usage {
            if let Some(pricing_rule) = self.pricing_rules.get(resource_type) {
                ...
                estimated_items.push(CostEstimateItem {
                    resource_type: resource_type.clone(),
                    usage: *usage,
                    unit_price: if *usage > 0 {
                        final_cost / *usage as f64
                    } else {
                        0.0
                    },
...
impl DiscountRule {
    ...
    pub fn applies_to_resource(&self, resource_type: &str) -> bool {
        self.conditions.resource_types.is_empty()
            || self
                .conditions
                .resource_types
    ...
    pub fn apply_discount(&self, cost: f64) -> BillingResult<f64> {
        match self.discount_type {
            DiscountType::Percentage => Ok(cost * (1.0 - self.value / 100.0)),
            DiscountType::FixedAmount => Ok((cost - self.value).max(0.0)),
            DiscountType::TrialExtension { .. } => {
                // 试用期延期不影响成本
                Ok(cost)
            }
