...
impl PaymentProcessor for MockPaymentProcessor {
    async fn process_payment(
        ...
        // 验证支付方式
        self.validate_payment_method(&payment_request.payment_method)
            .await?;
    ...
    async fn process_refund(
        ...
        if payment.status != PaymentStatus::Succeeded {
            return Err(BillingError::Other(
                "Cannot refund unsuccessful payment".to_string(),
            ));
        }
        ...
        let refund_record = RefundRecord {
            ...
            external_refund_id: Some(format!(
                "ref_{}",
                Uuid::new_v4().to_string().replace('-', "")
            )),
            metadata: HashMap::new(),
    ...
    async fn get_payment_history(&self, tenant_id: &Uuid) -> BillingResult<Vec<PaymentRecord>> {
        let history: Vec<PaymentRecord> = self
            .payment_records
            .values()
...
impl PaymentProcessor for StripePaymentProcessor {
    async fn process_payment(
        ...
        if self.test_mode {
            ...
            Err(BillingError::ExternalServiceError(
                "Stripe integration not implemented".to_string(),
            ))
        }
    }
    ...
    async fn process_refund(
        ...
        Err(BillingError::ExternalServiceError(
            "Stripe refund not implemented".to_string(),
        ))
    }
...
impl PaymentProcessorFactory {
    ...
    ) -> BillingResult<Box<dyn PaymentProcessor>> {
        match processor_type {
            "mock" => Ok(Box::new(MockPaymentProcessor::new())),
            "stripe" => {
                let api_key = config.get_string("api_key").ok_or_else(|| {
                    BillingError::InvalidConfiguration("Missing Stripe API key".to_string())
                })?;
                let test_mode = config.get_bool("test_mode").unwrap_or(true);
                Ok(Box::new(StripePaymentProcessor::new(api_key, test_mode)))
            }
            _ => Err(BillingError::InvalidConfiguration(format!(
