...
async fn test_complete_tenant_lifecycle() {
    ...
    // 测试资源分配
    let allocation_result = architecture
        .allocate_resources("tenant1", "cpu_cores", 1)
        .await;
    assert!(allocation_result.is_ok());

    // 测试配额超限
    let quota_exceeded = architecture
        .allocate_resources("tenant1", "cpu_cores", 10)
        .await;
...
async fn test_quota_management() {
    ...
    // 分配一些资源
    assert!(architecture
        .allocate_resources("quota_test", "cpu_cores", 1)
        .await
        .is_ok());
    assert!(architecture
        .allocate_resources("quota_test", "memory_gb", 2)
        .await
        .is_ok());
    ...
    // 测试配额超限
    let over_quota = architecture
        .allocate_resources("quota_test", "cpu_cores", 5)
        .await;
...
async fn test_auto_scaling() {
    ...
    let scaling_result = architecture
        .check_auto_scaling("scaling_test", 0.5, 0.6)
        .await
        .unwrap();
    ...
    // 测试高负载（需要扩容）
    let scaling_result = architecture
        .check_auto_scaling("scaling_test", 0.9, 0.85)
        .await
        .unwrap();
    ...
    // 测试低负载（需要缩容）
    let scaling_result = architecture
        .check_auto_scaling("scaling_test", 0.2, 0.3)
        .await
        .unwrap();
    assert!(scaling_result.is_some());
    ...
    // 检查扩容历史
    let history = architecture
        .get_scaling_history("scaling_test")
        .await
        .unwrap();
...
async fn test_billing_system() {
    ...
    // 分配一些资源（会自动记录计费）
    assert!(architecture
        .allocate_resources("billing_test", "cpu_cores", 2)
        .await
        .is_ok());
    assert!(architecture
        .allocate_resources("billing_test", "memory_gb", 4)
        .await
        .is_ok());
    assert!(architecture
        .allocate_resources("billing_test", "storage_gb", 50)
        .await
        .is_ok());
...
async fn test_resource_allocation_edge_cases() {
    ...
    // 测试零资源分配
    let zero_allocation = architecture
        .allocate_resources("edge_test", "cpu_cores", 0)
        .await;
    assert!(zero_allocation.is_ok());

    // 测试不存在的租户
    let nonexistent_tenant = architecture
        .allocate_resources("nonexistent", "cpu_cores", 1)
        .await;
    assert!(nonexistent_tenant.is_err());

    // 测试不支持的资源类型
    let unsupported_resource = architecture
        .allocate_resources("edge_test", "quantum_cores", 1)
        .await;
    // 这应该成功，因为我们支持自定义资源类型
...
async fn test_concurrent_operations() {
    ...
    for i in 0..5 {
        let allocation_result = architecture
            .allocate_resources("concurrent_test", "api_calls", 100)
            .await;
