...
mod tests {
    ...
    async fn test_regex_rule() {
        // 创建规则评估器
        let evaluator = RuleEvaluator::new("regex_evaluator").add_rule(Rule {
    ...
    async fn test_keywords_rule() {
        // 创建规则评估器
        let evaluator = RuleEvaluator::new("keywords_evaluator").add_rule(Rule {
        ...
        let result = evaluator
            .evaluate("", "Rust的所有权系统保证了内存安全", &options)
            .await
            .unwrap();
    ...
    async fn test_length_rule() {
        // 创建规则评估器
        let evaluator = RuleEvaluator::new("length_evaluator").add_rule(Rule {
        ...
        let options = EvalOptions::default();
        let result1 = evaluator
            .evaluate("", "这个回答长度适中，在允许范围内", &options)
            .await
            .unwrap();
        assert_eq!(result1.score, 1.0);
    ...
    async fn test_custom_rule() {
        // 创建规则评估器
        let evaluator = RuleEvaluator::new("custom_evaluator").add_rule(Rule {
