        ...
        for line in response.lines() {
            ...
            if line.starts_with(score_name) && (line.contains(':') || line.contains('：')) {
                ...
                let score: f64 = score_part.parse().map_err(|_| {
                    Error::MetricCalculation(format!("无法将'{}'解析为数字", score_part))
                })?;
...
impl Metric for SummarizationMetric {
    ...
    async fn measure(&self, input: &str, output: &str) -> Result<MetricResult> {
        ...
        // 提取评估结果
        let (faithfulness_score, coverage_score, total_score) =
            self.extract_scores_from_response(&response)?;
        ...
        info.insert(
            ...
            serde_json::Value::Number(serde_json::Number::from_f64(faithfulness_score).unwrap()),
        );
        info.insert(
            "coverage_score".to_string(),
            serde_json::Value::Number(serde_json::Number::from_f64(coverage_score).unwrap()),
        );
...
总分: 0.8
        ...
        let (faithfulness, coverage, total) =
            metric.extract_scores_from_response(response).unwrap();
...
Total Score: 0.8
        ...
        let (faithfulness, coverage, total) =
            metric.extract_scores_from_response(en_response).unwrap();
        assert_eq!(faithfulness, 0.9);
        assert_eq!(coverage, 0.8);
        assert_eq!(total, 0.8);
...
信息覆盖度分数: 0.7
        ...
        let (faithfulness, coverage, total) = metric
            .extract_scores_from_response(no_total_response)
            .unwrap();
        assert_eq!(faithfulness, 0.9);
