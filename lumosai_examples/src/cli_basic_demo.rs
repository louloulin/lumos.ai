...
fn demonstrate_available_commands() {
    ...
    let commands = vec![
        ("create", "创建新的 Lumosai 项目", "cargo run --bin lumosai_cli -- create --name my-project"),
        ("dev", "启动开发服务器", "cargo run --bin lumosai_cli -- dev"),
        ("ui", "启动 UI 服务器", "cargo run --bin lumosai_cli -- ui"),
        ("playground", "启动交互式测试环境", "cargo run --bin lumosai_cli -- playground"),
        ("api", "生成 API 端点", "cargo run --bin lumosai_cli -- api"),
        ("visualize", "生成可视化图表", "cargo run --bin lumosai_cli -- visualize"),
        ("monitoring", "启动监控服务器", "cargo run --bin lumosai_cli -- monitoring"),
    ];
