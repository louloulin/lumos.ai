...
use crate::analytics::{
    AnalyticsReport, DefaultUsageAnalytics, TimeRange, UsageAnalytics, UsageInfo, UserInfo,
};
use crate::config::MarketplaceConfig;
use crate::discovery::{
    DefaultToolDiscoveryEngine, SearchResult, ToolDiscoveryEngine, UserContext,
};
use crate::error::{MarketplaceError, Result};
use crate::models::{ToolCategory, ToolPackage};
use crate::publisher::{DefaultToolPublisher, PublishRequest, PublishResult, ToolPublisher};
use crate::registry::{RegistryStatistics, ToolRegistry};
use crate::search::{SearchEngine, SearchQuery, TantivySearchEngine};
use crate::security::{DefaultSecurityScanner, SecurityScanner};
use crate::storage::{SqliteStorage, Storage};
use crate::validator::{DefaultToolValidator, ToolValidator, ValidationResult};
...
impl ToolMarketplace {
    ...
    pub async fn new(config: MarketplaceConfig) -> Result<Self> {
        ...
        let search_engine: Arc<dyn SearchEngine> =
            Arc::new(TantivySearchEngine::new(&config.search_index_path).await?);
        ...
        // 创建发现引擎
        let discovery: Arc<dyn ToolDiscoveryEngine> = Arc::new(DefaultToolDiscoveryEngine::new(
            storage.clone(),
            search_engine.clone(),
        ));
    ...
    /// 根据名称和版本获取工具包
    pub async fn get_package_by_name_version(
        &self,
        name: &str,
        version: &str,
    ) -> Result<Option<ToolPackage>> {
        self.registry
            .get_package_by_name_version(name, version)
            .await
    }
    ...
    pub async fn rate_package(&self, id: Uuid, rating: f64, user_info: &UserInfo) -> Result<()> {
        ...
        self.registry
            .update_rating(id, new_rating, new_count)
            .await?;
    ...
    pub async fn get_tool_statistics(
        &self,
        package_id: Uuid,
    ) -> Result<crate::analytics::UsageStatistics> {
    ...
    /// 扫描工具包安全性
    pub async fn scan_package_security(
        &self,
        package: &ToolPackage,
    ) -> Result<crate::models::SecurityAuditResult> {
...
mod tests {
    ...
    async fn test_invalid_rating() {
        ...
        let result = marketplace
            .rate_package(Uuid::new_v4(), 6.0, &user_info)
            .await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("评分必须在1.0-5.0之间"));
    }
}
