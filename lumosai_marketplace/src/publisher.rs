...
use crate::security::{SecurityAuditResult, SecurityScanner};
...
pub trait ToolPublisher: Send + Sync {
    ...
    /// 验证发布权限
    async fn verify_publish_permission(
        &self,
...
impl DefaultToolPublisher {
    ...
    async fn validate_package_file(&self, file_path: &str) -> Result<()> {
        ...
        match path.extension().and_then(|ext| ext.to_str()) {
            ...
            _ => Err(MarketplaceError::publish(
                "不支持的文件格式，请使用 .tar.gz 或 .zip",
            )),
        }
    }
...
impl ToolPublisher for DefaultToolPublisher {
    async fn publish(&self, request: PublishRequest) -> Result<PublishResult> {
        ...
        // 验证发布权限
        if !self
            .verify_publish_permission(&request.publisher_info, &request.package)
            .await?
        {
        ...
        // 上传工具包文件
        match self
            .upload_package_file(package.id, &request.package_path)
            .await
        {
            Ok(_) => {
                // 标记为已发布
...
mod tests {
    ...
    async fn test_publisher_creation() {
        ...
        let has_permission = publisher
            .verify_publish_permission(&publisher_info, &package)
            .await
            .unwrap();
    ...
    async fn test_package_file_validation() {
        ...
        let result = publisher
            .validate_package_file(temp_file.path().to_str().unwrap())
            .await;
        assert!(result.is_ok());

        // 测试不存在的文件
        let result = publisher
            .validate_package_file("/nonexistent/file.tar.gz")
            .await;
    ...
    async fn test_invalid_publisher_info() {
        ...
        let package = create_test_package();
        let has_permission = publisher
            .verify_publish_permission(&invalid_publisher_info, &package)
            .await
            .unwrap();
        assert!(!has_permission);
    }
