...
use tantivy::{
    ...
    schema::{Field, Schema, FAST, STORED, STRING, TEXT, U64},
...
impl TantivySearchEngine {
    ...
    fn package_to_document(&self, package: &ToolPackage) -> Document {
        ...
        let categories_text: Vec<String> = package
            .categories
            .iter()
        ...
        doc.add_text(
            self.fields.published,
            if package.published { "true" } else { "false" },
        );
        doc.add_text(
            self.fields.verified,
            if package.verified { "true" } else { "false" },
        );
        doc.add_u64(
            self.fields.created_at,
            package.created_at.timestamp() as u64,
        );
        doc.add_u64(
            self.fields.updated_at,
            package.updated_at.timestamp() as u64,
        );
