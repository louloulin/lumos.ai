...
use crate::models::{
    SecurityAuditResult, SecurityIssue, SecurityIssueType, SecurityLevel, Severity, ToolPackage,
};
...
pub trait SecurityScanner: Send + Sync {
    ...
    /// 扫描权限
    async fn scan_permissions(
        &self,
        permissions: &[crate::models::Permission],
    ) -> Result<Vec<SecurityIssue>>;
}
...
impl SecurityScanner for DefaultSecurityScanner {
    async fn scan_package(&self, package: &ToolPackage) -> Result<SecurityAuditResult> {
        ...
        let metadata_code = format!(
            "{} {} {}",
            package.name, package.description, package.author
        );
    ...
    async fn scan_dependencies(
        ...
        for (dep_name, version) in dependencies {
            ...
            if self
                .known_vulnerabilities
                .iter()
                .any(|vuln| dep_name.contains(vuln))
            {
                issues.push(SecurityIssue {
            ...
            match permission.risk_level() {
                crate::models::RiskLevel::High => {
                    issues.push(SecurityIssue {
                        ...
                        line_number: None,
                        fix_suggestion: Some(
                            "请确保此权限是必需的，并在文档中说明用途".to_string(),
                        ),
                    });
                }
...
mod tests {
    ...
    async fn test_dependency_scanning() {
        ...
        assert!(issues
            .iter()
            .any(|i| i.description.contains("不精确的版本号")));
    }
