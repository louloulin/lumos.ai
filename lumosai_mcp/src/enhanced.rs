...
impl EnhancedMCPManager {
    ...
    async fn connect_client(&self, name: &str) -> Result<()> {
        ...
        match result {
            ...
            Err(_) => {
                let timeout_error =
                    MCPError::TimeoutError(self.config.connection_timeout.as_millis() as u64);
                status.is_healthy = false;
                status.consecutive_failures += 1;
    ...
    pub async fn execute_tool(
        ...
        while attempts < self.config.max_retry_attempts {
            match client
                .execute_tool("default_resource", tool_name, parameters.clone(), false)
                .await
            {
                Ok(result) => {
                    // Update metrics
                    self.update_success_metrics(&client_name, tool_name, start_time.elapsed())
                        .await;
                    return Ok(result);
                }
                Err(e) => {
        ...
        self.update_failure_metrics(&client_name, tool_name, &error)
            .await;
        self.mark_client_unhealthy(&client_name, &error.to_string())
            .await;
        Err(error)
    }
    ...
    async fn update_success_metrics(
        ...
        *metrics
        ...
        let total_time = metrics.average_response_time.as_nanos() as f64
            * (metrics.successful_requests - 1) as f64;
        let new_average =
            (total_time + response_time.as_nanos() as f64) / metrics.successful_requests as f64;
                ...
                match self.discover_tools_from_server(server_name, client).await {
                    Ok(tools) => {
                        ...
                        println!(
                            ...
                            discovered_tools.get(server_name).unwrap().len(),
                            server_name
                        );
                    }
                    Err(e) => {
    ...
    async fn discover_tools_from_server(
        ...
        {
            let mut cache = self.tool_cache.write().await;
            let tool_cache_entries: Vec<Tool> = all_tools
                .iter()
                .map(|tool_def| Tool {
    ...
    async fn get_tools_from_server_direct(
        &self,
        server_name: &str,
        client: &MCPClient,
    ) -> Result<Vec<ToolDefinition>> {
    ...
    async fn discover_tools_from_server_by_name(&self, server_name: &str) -> Result<()> {
        ...
        self.discover_tools_from_server(server_name, &client)
            .await?;
        Ok(())
    }
    ...
    pub async fn execute_mcp_tool(
        ...
        while attempts < self.config.max_retry_attempts {
            ...
            if !self.is_client_healthy(&server_name).await && attempts > 0 {
                ...
                if let Ok(alt_server) = self
                    .find_alternative_server_for_tool(tool_name, &server_name)
                    .await
                {
                    return self
                        .execute_on_server(&alt_server, tool_name, &params, start_time)
                        .await;
                }
            }

            match self
                .execute_on_server(&server_name, tool_name, &params, start_time)
                .await
            {
                Ok(result) => {
                    self.update_success_metrics(&server_name, tool_name, start_time.elapsed())
                        .await;
                    return Ok(result);
                }
                Err(e) => {
        ...
        // All attempts failed
        let error = last_error.unwrap();
        self.update_failure_metrics(&server_name, tool_name, &error)
            .await;
        self.mark_client_unhealthy(&server_name, &error.to_string())
            .await;
        Err(error)
    }
    ...
    async fn find_best_server_for_tool(&self, tool_name: &str) -> Result<String> {
        ...
        candidates.sort_by(|a, b| {
            let a_response_time = health_status
                .get(a)
                .and_then(|s| s.response_time)
                .unwrap_or(Duration::from_secs(999));
            let b_response_time = health_status
                .get(b)
                .and_then(|s| s.response_time)
                .unwrap_or(Duration::from_secs(999));
    ...
    async fn find_alternative_server_for_tool(
        &self,
        tool_name: &str,
        exclude_server: &str,
    ) -> Result<String> {
