...
use crate::types::{MCPMessage, SSEServerParameters, ServerParameters, StdioServerParameters};
...
impl Transport for StdioTransport {
    ...
    async fn receive_message(&mut self) -> Result<MCPMessage> {
        ...
        if let Some(tx) = &*tx_guard {
            ...
            match rx.recv().await {
                Ok(result) => result,
                Err(_) => Err(MCPError::ConnectionError(
...
impl Transport for SSETransport {
    async fn connect(&mut self) -> Result<()> {
        ...
        let message_stream = event_stream.filter_map(|event_result| async move {
            match event_result {
                Ok(event) => {
