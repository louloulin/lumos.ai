...
use crate::types::{AgentCapability, AgentId, AgentLocation, AgentType};
    ...
    fn start_expiry_check(discovery: Arc<Self>) {
        tokio::spawn(async move {
            ...
            loop {
                ...
                let expired_ids: Vec<AgentId> = discovery
                    .registrations
                    .iter()
...
impl ServiceDiscovery for InMemoryServiceDiscovery {
    async fn register(&self, registration: ServiceRegistration) -> Result<()> {
        ...
        // 通知监听器
        if is_update {
            self.notify_listeners(ServiceEvent::Updated(registration))
                .await;
        } else {
            self.notify_listeners(ServiceEvent::Registered(registration))
                .await;
        }
    ...
    async fn deregister(&self, id: &AgentId) -> Result<()> {
        if let Some((_, _)) = self.registrations.remove(id) {
            // 通知监听器
            self.notify_listeners(ServiceEvent::Deregistered(id.clone()))
                .await;
            Ok(())
        } else {
    ...
    async fn discover(&self, query: &ServiceQuery) -> Result<Vec<ServiceRegistration>> {
        ...
        for entry in self.registrations.iter() {
            ...
            let metadata_match = query
                .metadata_filters
                .iter()
                .all(|(key, value)| registration.metadata.get(key).map_or(false, |v| v == value));
...
mod tests {
    ...
    async fn test_service_events() {
        ...
        // 设置监听器
        {
            ...
            discovery
                .set_listener(Box::new(move |event| match event {
    ...
    async fn test_service_expiry() {
        ...
        let registration =
            ServiceRegistration::new(agent_id.clone(), AgentType::Regular).with_ttl(1); // 1秒TTL
    ...
    async fn test_heartbeat() {
        ...
        let registration =
            ServiceRegistration::new(agent_id.clone(), AgentType::Regular).with_ttl(2); // 2秒TTL
