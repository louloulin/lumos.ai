...
use crate::discovery::{InMemoryServiceDiscovery, ServiceDiscovery, ServiceRegistration};
use crate::error::{Error, Result};
use crate::message::{Message, MessageType};
use crate::router::{DefaultMessageRouter, MessageRouter};
use crate::topology::{GraphTopology, NetworkTopology};
use crate::types::{AgentCapability, AgentId, AgentStatus, AgentType};
        ...
        F: Fn(Message) -> Result<Vec<Message>> + Send + Sync + 'static,
    {
        ...
        if let Some(mut handlers) = self.message_handlers.get_mut(&message_type) {
            ...
            self.message_handlers
                .insert(message_type, vec![handler_box]);
        }
    }
...
impl AgentNetwork {
    ...
    pub async fn add_agent(&self, mut agent: AgentNode) -> Result<Arc<AgentNode>> {
        ...
        if agent_arc.config.register_with_discovery {
            let registration =
                ServiceRegistration::new(agent_id.clone(), agent_arc.config.agent_type.clone())
    ...
    pub async fn broadcast(
        ...
        // 创建广播消息
        let receivers: Vec<AgentId> = self
            .agents
            .iter()
        ...
        let message = Message::new(sender.clone(), receivers, message_type, content.into());
...
pub struct NetworkManager {
    ...
    /// 用于管理消息处理器的映射
    message_handlers:
        DashMap<MessageType, Vec<Box<dyn Fn(Message) -> Result<Vec<Message>> + Send + Sync>>>,
    /// 网络拓扑管理
