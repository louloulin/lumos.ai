...
impl NetworkTopology for GraphTopology {
    async fn add_node(&self, id: AgentId, attrs: Option<NodeAttributes>) -> Result<()> {
        ...
        if self.topology_type == TopologyType::FullyConnected {
            // 获取所有现有节点
            let existing_nodes: Vec<(AgentId, NodeIndex)> = indices
                .iter()
        ...
        while current != from_idx {
            ...
            for edge in prev_edges {
                ...
                let edge_weight = graph
                    .edge_weight(graph.find_edge(source, current).unwrap())
                    .unwrap();

                if path.contains_key(&source)
                    && path[&source] + edge_weight.weight == path[&current]
                {
...
mod tests {
    ...
    async fn test_edge_weights() {
        ...
        // node1 -> node2 (权重=1)
        topology
            .add_edge(&node1, &node2, Some(edge_attrs1.clone()))
            .await
            .unwrap();

        // node1 -> node3 (权重=10)
        topology
            .add_edge(&node1, &node3, Some(edge_attrs2))
            .await
            .unwrap();

        // node2 -> node3 (权重=1)
        topology
            .add_edge(&node2, &node3, Some(edge_attrs1))
            .await
            .unwrap();
