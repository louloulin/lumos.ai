...
use crate::embedding::provider::{utils, EmbeddingProvider};
...
impl EmbeddingProvider for OpenAIEmbeddingProvider {
    ...
    async fn embed_text(&self, text: &str) -> Result<Vec<f32>> {
        ...
        embeddings
            .into_iter()
            .next()
            .ok_or_else(|| RagError::Embedding("No embedding returned from API".to_string()))
    }
...
mod tests {
    ...
    async fn test_cosine_similarity() {
        let provider =
            OpenAIEmbeddingProvider::new("test-key".to_string(), "test-model".to_string());
