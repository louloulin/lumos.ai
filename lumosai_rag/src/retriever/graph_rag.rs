...
impl KnowledgeGraph {
    ...
    pub async fn add_entity(&self, entity: Entity) -> Result<()> {
        ...
        // 添加实体
        self.entities.write().await.insert(entity_id.clone(), entity.clone());
        
        // 初始化邻接表
        self.adjacency.write().await.entry(entity_id.clone()).or_insert_with(Vec::new);
    ...
    pub async fn add_relation(&self, relation: Relation) -> Result<()> {
        ...
        // 添加关系
        self.relations.write().await.insert(relation_id.clone(), relation);
...
impl GraphRagRetriever {
    ...
    pub async fn add_document(&self, document: Document) -> Result<()> {
        ...
        // 存储文档
        self.documents.write().await.insert(doc_id.clone(), document.clone());
    ...
    /// 从文档中提取关系（简化实现）
    async fn extract_relations(&self, _document: &Document, entities: &[Entity]) -> Result<Vec<Relation>> {
        // 简化实现：基于共现创建关系
        // 实际应使用关系抽取模型
    ...
    async fn retrieve(&self, request: &RetrievalRequest) -> Result<RetrievalResult> {
        ...
        for query_entity in &query_entities {
            ...
            for (entity_id, entity) in entities.iter() {
                if entity.name.to_lowercase().contains(&query_entity.to_lowercase()) {
        ...
        for entity_id in &relevant_entity_ids {
            let neighbors = self.graph.get_neighbors(entity_id, self.config.max_traversal_depth).await;
