...
use crate::embedding::{utils, EmbeddingProvider};
...
impl InMemoryVectorStore {
    ...
    fn find_similar_documents(
        ...
        let documents = scored_docs
            .iter()
            .map(|(score, doc)| crate::types::ScoredDocument {
                document: (*doc).clone(),
                score: *score,
            })
            .collect();
...
mod tests {
    ...
    async fn test_query_by_vector() {
        ...
        let result = store
            .query_by_vector(&query_vector, &options)
            .await
            .unwrap();
    ...
    async fn test_query_by_text() {
        ...
        let results = store
            .query_by_text("test query", &options, &mock_provider)
            .await
            .unwrap();
        ...
        assert!(
            results.documents[0].document.id == "doc-1"
                || results.documents[0].document.id == "doc-2"
        );
    }
