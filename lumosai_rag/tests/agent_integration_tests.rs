...
use lumosai_rag::{
    ...
    types::{ChunkingConfig, ChunkingStrategy, Document, Metadata},
    RagError, RagPipeline, RagPipelineBuilder,
};
...
async fn test_rag_pipeline_with_different_chunking_strategies() {
    ...
    let chunks = markdown_pipeline
        .process_document(markdown_doc)
        .await
        .unwrap();
...
async fn test_rag_pipeline_json_chunking() {
    ...
    let all_content: String = chunks
        ...
        .join(" ");
    assert!(
        all_content.contains("users")
            || all_content.contains("settings")
            || all_content.contains("metadata")
    );
...
async fn test_rag_pipeline_metadata_extraction() {
    ...
    let summary = test_doc
        .metadata
        .fields
        .get("extracted_summary")
        .unwrap()
        .as_str()
        .unwrap();
    assert!(!summary.is_empty());

    let keywords = test_doc
        .metadata
        .fields
