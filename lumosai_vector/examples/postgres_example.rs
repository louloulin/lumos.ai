...
async fn main() -> Result<()> {
    ...
    let config = lumosai_vector::postgres::PostgresConfig::new(&database_url).with_performance(
        lumosai_vector::postgres::config::PerformanceConfig {
            batch_size: 1000,
    ...
    let documents = vec![
        ...
        Document {
            id: "doc_3".to_string(),
            content: "Climate change is a pressing global issue requiring immediate action."
                .to_string(),
    ...
    println!("\n🧹 Cleaning up...");
    storage
        .delete_documents(index_name, vec!["doc_2".to_string(), "doc_4".to_string()])
        .await?;
