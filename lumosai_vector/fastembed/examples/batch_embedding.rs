...
use lumosai_vector_fastembed::{FastEmbedConfigBuilder, FastEmbedModel, FastEmbedProvider};
...
async fn await_parallel_comparison(data: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for chunk in chunks {
        ...
        let handle = tokio::spawn(async move { provider.embed_batch(&chunk_data).await });
