    ...
    async fn ensure_model_loaded(&self) -> Result<()> {
        ...
        if model_guard.is_none() {
            ...
            let mut init_options =
                fastembed::InitOptions::new(self.model_config.to_fastembed_model())
            ...
            let embedding_model = fastembed::TextEmbedding::try_new(init_options).map_err(|e| {
                FastEmbedError::ModelInitialization(format!(
...
impl EmbeddingModel for FastEmbedProvider {
    ...
    async fn embed_text(
        &self,
        text: &str,
    ) -> std::result::Result<Vector, lumosai_vector_core::error::VectorError> {
    ...
    async fn embed_batch(
        &self,
        texts: &[String],
    ) -> std::result::Result<Vec<Vector>, lumosai_vector_core::error::VectorError> {
        if texts.is_empty() {
            return Ok(Vec::new());
        }
    ...
    async fn health_check(
        &self,
    ) -> std::result::Result<(), lumosai_vector_core::error::VectorError> {
