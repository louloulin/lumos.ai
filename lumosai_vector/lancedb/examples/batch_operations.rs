...
use lumosai_vector_lancedb::{LanceDbConfigBuilder, LanceDbStorage};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "\n💾 Inserting {} documents in batches of {}...",
    ...
    for batch_idx in 0..num_batches {
        ...
        let document_ids = storage
            .upsert_documents("batch_documents", documents)
            .await?;
    ...
    storage
        .delete_documents("batch_documents", ids_to_delete.clone())
        .await?;
    ...
    println!(
        "   Backend: {} v{}",
        backend_info.name, backend_info.version
    );
...
fn generate_document_batch(start_id: usize, batch_size: usize, vector_dim: usize) -> Vec<Document> {
    ...
    for i in 0..batch_size {
        ...
        let content = format!(
            "This is document number {} with some sample content for testing batch operations.",
            doc_id
        );
