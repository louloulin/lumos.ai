...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    storage
        .upsert_documents("search_documents", documents)
        .await?;
    ...
    for result in &response.results {
        ...
        println!(
            "   - {}: score {:.4}, words: {}",
            result.id, result.score, word_count
        );
    }
    ...
    println!(
        "   Unfiltered: {:.2}ms avg ({} queries)",
        unfiltered_time.as_millis() as f64 / num_queries as f64,
        num_queries
    );
    println!(
        "   Filtered: {:.2}ms avg ({} queries)",
        filtered_time.as_millis() as f64 / num_queries as f64,
        num_queries
    );
    println!(
        "   Overhead: {:.1}%",
