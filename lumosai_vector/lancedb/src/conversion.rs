...
use arrow::array::{Array, Float32Array, Float32Builder, ListBuilder, StringArray};
    ...
    for doc in documents {
        ...
        if let Some(embedding) = &doc.embedding {
            ...
            return Err(LanceDbError::InvalidData(
                "Document missing embedding".to_string(),
            ));
        }
...
pub fn metadata_value_to_json(value: &MetadataValue) -> serde_json::Value {
    match value {
        ...
        MetadataValue::Array(arr) => {
            let json_arr: Vec<serde_json::Value> = arr.iter().map(metadata_value_to_json).collect();
...
pub fn json_to_metadata_value(value: &serde_json::Value) -> LanceDbResult<MetadataValue> {
    match value {
        ...
        serde_json::Value::Array(arr) => {
            let metadata_arr: Result<Vec<MetadataValue>, _> =
...
pub fn validate_vector_dimension(vectors: &[Vec<f32>]) -> LanceDbResult<usize> {
    ...
    for (i, vector) in vectors.iter().enumerate() {
        if vector.len() != expected_dim {
            return Err(LanceDbError::InvalidData(format!(
