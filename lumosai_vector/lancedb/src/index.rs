    ...
    pub fn create_index(&self, config: &IndexConfiguration) -> LanceDbResult<Index> {
        match config.index_type {
            ...
            IndexType::HNSW => {
                ...
                Err(LanceDbError::Index(
                    "HNSW index not supported in current LanceDB version".to_string(),
                ))
            }
            ...
            IndexType::LSH => {
                ...
                Err(LanceDbError::Index(
                    "LSH index not supported in current LanceDB version".to_string(),
                ))
            }

            IndexType::None => Err(LanceDbError::Index(
                "Cannot create index of type None".to_string(),
            )),
        }
    }
    ...
    pub fn calculate_optimal_params(
        ...
        match index_type {
            IndexType::IVF => {
                ...
                let num_clusters = ((vector_count as f64).sqrt() as usize).max(16).min(65536);
        ...
        match index_type {
            IndexType::IVF => {
                if let Some(clusters) = params.num_clusters {
                    ...
                    if clusters > 1_000_000 {
                        return Err(LanceDbError::InvalidConfiguration(
                            "Number of clusters too large".to_string(),
                        ));
                    }
                }
            }
            ...
            IndexType::IVFPQ => {
                ...
                if let Some(sub_quantizers) = params.num_sub_quantizers {
                    if sub_quantizers == 0 {
                        return Err(LanceDbError::InvalidConfiguration(
                            "Number of sub-quantizers must be greater than 0".to_string(),
                        ));
                    }
                    if sub_quantizers > 256 {
                        return Err(LanceDbError::InvalidConfiguration(
                            "Too many sub-quantizers".to_string(),
                        ));
                    }
                }
                ...
                if let Some(bits) = params.bits_per_sub_quantizer {
                    if bits == 0 || bits > 16 {
                        return Err(LanceDbError::InvalidConfiguration(
                            "Bits per sub-quantizer must be between 1 and 16".to_string(),
                        ));
                    }
                }
            }
            ...
            IndexType::HNSW => {
                if let Some(m) = params.hnsw_m {
                    if m == 0 || m > 128 {
                        return Err(LanceDbError::InvalidConfiguration(
                            "HNSW M parameter must be between 1 and 128".to_string(),
                        ));
                    }
                }
            ...
            IndexType::LSH => {
                if let Some(tables) = params.lsh_num_tables {
                    if tables == 0 || tables > 100 {
                        return Err(LanceDbError::InvalidConfiguration(
                            "LSH number of tables must be between 1 and 100".to_string(),
                        ));
                    }
                }
                ...
                if let Some(functions) = params.lsh_num_functions {
                    if functions == 0 || functions > 50 {
                        return Err(LanceDbError::InvalidConfiguration(
                            "LSH number of functions must be between 1 and 50".to_string(),
                        ));
                    }
                }
            }
...
mod tests {
    ...
    fn test_params_validation() {
        ...
        assert!(manager
            .validate_params(&IndexType::IVF, &valid_params)
            .is_ok());
        ...
        assert!(manager
            .validate_params(&IndexType::IVF, &invalid_params)
            .is_err());
    }
}
