...
impl LanceDbStorage {
    ...
    async fn get_or_create_schema(
        &self,
        index_name: &str,
        vector_dim: usize,
    ) -> LanceDbResult<Schema> {
    ...
    fn format_value(&self, value: &MetadataValue) -> LanceDbResult<String> {
        match value {
            ...
            MetadataValue::Array(arr) => {
                let formatted: std::result::Result<Vec<String>, LanceDbError> =
                    arr.iter().map(|v| self.format_value(v)).collect();
                Ok(format!("[{}]", formatted?.join(", ")))
            }
            MetadataValue::Object(_) => Err(LanceDbError::InvalidData(
...
impl VectorStorage for LanceDbStorage {
    ...
    async fn create_index(&self, config: IndexConfig) -> Result<()> {
        ...
        {
            return Err(LanceDbError::already_exists(format!(
        ...
        let schema = self
            .get_or_create_schema(&config.name, config.dimension)
            .await
            .map_err(VectorError::from)?;
        ...
        if self.config.index_config.auto_create_index {
            match self.config.index_config.default_index_type {
                ...
                crate::config::IndexType::IVFPQ => {
                    ...
                    if let Some(num_sub_quantizers) =
                        self.config.index_config.index_params.num_sub_quantizers
                    {
    ...
    async fn upsert_documents(
        ...
        for doc in &documents {
            if doc.embedding.is_none() {
                return Err(LanceDbError::invalid_data(format!(
        ...
        let schema = self
            .get_or_create_schema(index_name, vector_dim)
            .await
            .map_err(VectorError::from)?;

        // Convert documents to record batch
        let batch =
            utils::documents_to_record_batch(&documents, &schema).map_err(VectorError::from)?;
    ...
    async fn search(&self, request: SearchRequest) -> Result<SearchResponse> {
        ...
        if !self
            .client
            .table_exists(&request.index_name)
            .await
            .map_err(VectorError::from)?
        {
            return Err(LanceDbError::not_found(format!(
                "Index '{}' not found",
                request.index_name
            ))
            .into());
        ...
        let vector = match &request.query {
            ...
            SearchQuery::Text(_) => {
                return Err(
                    LanceDbError::InvalidData("Text search not supported yet".to_string()).into(),
                )
            }
        };
    ...
    fn backend_info(&self) -> BackendInfo {
        BackendInfo::new("lancedb", "0.8.0")
            ...
            .with_metadata(
                "compression_enabled",
                self.config.performance.enable_compression,
            )
    }
}
