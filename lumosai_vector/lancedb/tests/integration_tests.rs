...
use lumosai_vector_core::{
    ...
    types::{
        Document, FilterCondition, IndexConfig, MetadataValue, SearchRequest, SimilarityMetric,
    },
};
use lumosai_vector_lancedb::{LanceDbConfig, LanceDbConfigBuilder, LanceDbStorage};
...
async fn create_test_storage() -> (LanceDbStorage, TempDir) {
    ...
    let storage = LanceDbStorage::new(config)
        .await
...
async fn test_document_operations() {
    ...
    let doc_ids = storage
        .upsert_documents("docs", documents)
        .await
    ...
    let retrieved = storage
        .get_documents("docs", vec!["doc1".to_string(), "doc2".to_string()], true)
        .await
    ...
    let retrieved_no_vectors = storage
    ...
    storage
        .update_document("docs", updated_doc)
        .await
    ...
    storage
        .delete_documents("docs", vec!["doc3".to_string()])
        .await
        .expect("Failed to delete document");

    let remaining = storage
        .get_documents("docs", vec!["doc3".to_string()], false)
        .await
...
async fn test_vector_search() {
    ...
    storage
        .upsert_documents("search_test", documents)
        .await
        .expect("Failed to insert documents");
...
async fn test_complex_filters() {
    ...
    storage
        .upsert_documents("filter_test", documents)
        .await
        .expect("Failed to insert documents");
    ...
    let and_response = storage
        .search(and_search)
        .await
        .expect("Failed to search with AND filter");
    ...
    let or_response = storage
        .search(or_search)
        .await
        .expect("Failed to search with OR filter");
    ...
    let range_response = storage
        .search(range_search)
        .await
        .expect("Failed to search with range filter");
...
async fn test_configuration() {
    ...
    let storage = LanceDbStorage::new(config)
        .await
        .expect("Failed to create storage with custom config");
    storage.health_check().await.expect("Health check failed");
}
...
async fn test_batch_operations() {
    ...
    let doc_ids = storage
        .upsert_documents("batch_test", documents)
        .await
    ...
    let response = storage
        .search(search_request)
        .await
        .expect("Failed to search batch");
    assert_eq!(response.results.len(), 10);

    // Test batch deletion
    let ids_to_delete: Vec<String> = (0..10).map(|i| format!("batch_doc_{}", i)).collect();
    storage
        .delete_documents("batch_test", ids_to_delete)
        .await
        .expect("Failed to delete batch");

    // Verify deletion
    let remaining = storage
        .get_documents("batch_test", vec!["batch_doc_0".to_string()], false)
        .await
