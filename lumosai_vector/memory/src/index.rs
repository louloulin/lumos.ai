...
use lumosai_vector_core::traits::{filter::StandardFilterEvaluator, similarity};
    ...
    fn estimate_metadata_value_size(&self, value: &MetadataValue) -> u64 {
        match value {
            ...
            MetadataValue::Array(arr) => arr
                .iter()
                .map(|v| self.estimate_metadata_value_size(v))
                .sum(),
            MetadataValue::Object(obj) => obj
                .iter()
                .map(|(k, v)| k.len() as u64 + self.estimate_metadata_value_size(v))
                .sum(),
