...
impl VectorStorage for MemoryVectorStorage {
    ...
    async fn search(&self, request: SearchRequest) -> Result<SearchResponse> {
        ...
        if let Some(cached_response) = self.search_cache.get(&cache_key).await {
            ...
            self.performance_monitor
                .record_operation(duration, true)
                .await;
        ...
        let response = SearchResponse::new(results).with_execution_time(execution_time_ms);
    ...
    fn backend_info(&self) -> BackendInfo {
        BackendInfo::new("memory", env!("CARGO_PKG_VERSION"))
            ...
            .with_metadata(
                "initial_capacity",
                MetadataValue::Integer(self.config.initial_capacity as i64),
            )
            .with_metadata(
                "approximate_search",
                MetadataValue::Boolean(self.config.enable_approximate),
            )
    }
}
