...
use lumosai_vector_core::{
    ...
    types::{Document, IndexConfig, SearchQuery, SearchRequest, SimilarityMetric},
};
use lumosai_vector_milvus::{MilvusConfig, MilvusStorage};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match storage.search(filtered_search_request).await {
        Ok(filtered_response) => {
            println!(
                "✅ Filtered search completed! Found {} technology documents:",
                filtered_response.results.len()
            );
    ...
    let updated_doc = Document::new(
        "doc1",
        "The quick brown fox jumps over the lazy dog (updated)",
    )
    .with_embedding(generate_sample_embedding(384, 1))
    .with_metadata("category", "animals")
    ...
    println!("\n🗑️  Deleting document...");
    match storage
        .delete_documents("documents", vec!["doc5".to_string()])
        .await
    {
