...
impl MilvusClient {
    ...
    pub async fn create_collection(&self, schema: CollectionSchema) -> MilvusResult<()> {
        ...
        let request = CreateCollectionRequest {
            ...
            consistency_level: format!(
                "{:?}",
                self.connection.config().collection_config.consistency_level
            ),
        };

        let response = self.connection.post(&url).json(&request).send().await?;
    ...
    pub async fn drop_collection(&self, collection_name: &str) -> MilvusResult<()> {
        ...
        let response = self.connection.delete(&url).send().await?;
    ...
    pub async fn create_index(
        ...
        if response.status().is_success() {
            tracing::info!(
                "Index created for collection '{}' field '{}'",
                collection_name,
                field_name
            );
            Ok(())
    ...
    pub async fn insert(
        ...
        if response.status().is_success() {
            tracing::debug!(
                "Inserted {} entities into collection '{}'",
                entities.len(),
                collection_name
            );
            Ok(())
