...
impl From<MilvusError> for lumosai_vector_core::error::VectorError {
    fn from(err: MilvusError) -> Self {
        match err {
            MilvusError::Connection(msg) => {
                lumosai_vector_core::error::VectorError::ConnectionFailed(msg)
            }
            MilvusError::Authentication(msg) => {
                lumosai_vector_core::error::VectorError::AuthenticationFailed(msg)
            }
            MilvusError::Database(msg) => {
                lumosai_vector_core::error::VectorError::OperationFailed(msg)
            }
            MilvusError::Collection(msg) => {
                lumosai_vector_core::error::VectorError::OperationFailed(msg)
            }
            MilvusError::Index(msg) => {
                lumosai_vector_core::error::VectorError::OperationFailed(msg)
            }
            MilvusError::Query(msg) => {
                lumosai_vector_core::error::VectorError::OperationFailed(msg)
            }
            MilvusError::InvalidData(msg) => {
                lumosai_vector_core::error::VectorError::InvalidVector(msg)
            }
            MilvusError::InvalidConfiguration(msg) => {
                lumosai_vector_core::error::VectorError::OperationFailed(msg)
            }
            MilvusError::Serialization(msg) => {
            ...
            MilvusError::Timeout => lumosai_vector_core::error::VectorError::OperationFailed(
                "Operation timed out".to_string(),
            ),
            MilvusError::NotFound(msg) => {
                lumosai_vector_core::error::VectorError::IndexNotFound(msg)
            }
            MilvusError::AlreadyExists(msg) => {
                lumosai_vector_core::error::VectorError::IndexAlreadyExists(msg)
            }
            MilvusError::PermissionDenied(msg) => {
                lumosai_vector_core::error::VectorError::AuthenticationFailed(msg)
            }
            MilvusError::RateLimitExceeded(msg) => {
                lumosai_vector_core::error::VectorError::OperationFailed(msg)
            }
            MilvusError::ServiceUnavailable(msg) => {
                lumosai_vector_core::error::VectorError::ConnectionFailed(msg)
            }
            MilvusError::Generic(msg) => {
                lumosai_vector_core::error::VectorError::OperationFailed(msg)
            }
        }
    }
