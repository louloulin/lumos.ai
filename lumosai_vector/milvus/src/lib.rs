...
pub async fn create_milvus_storage_with_config(
    config: MilvusConfig,
) -> MilvusResult<MilvusStorage> {
    MilvusStorage::new(config).await
}
    ...
    pub fn documents_to_entities(documents: &[Document]) -> MilvusResult<Vec<MilvusEntity>> {
        ...
        for doc in documents {
            let embedding = doc.embedding.as_ref().ok_or_else(|| {
                MilvusError::InvalidData("Document missing embedding".to_string())
            })?;
        ...
        match value {
            ...
            MetadataValue::Array(arr) => {
                let json_arr: Vec<serde_json::Value> =
                    arr.iter().map(metadata_value_to_milvus).collect();
    ...
    pub fn validate_vector_dimension(vectors: &[Vec<f32>]) -> MilvusResult<usize> {
        ...
        for (i, vector) in vectors.iter().enumerate() {
            if vector.len() != expected_dim {
                return Err(MilvusError::InvalidData(format!(
