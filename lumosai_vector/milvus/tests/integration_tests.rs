...
use lumosai_vector_milvus::{MilvusConfig, MilvusConfigBuilder, MilvusStorage};
...
async fn test_document_operations() {
    if let Some(storage) = create_test_storage().await {
        ...
        let retrieved_no_vectors = storage
            .get_documents(collection_name, vec!["doc1".to_string()], false)
            .await
            .unwrap();
        ...
        assert!(storage
            .update_document(collection_name, updated_doc)
            .await
            .is_ok());

        // Test document deletion
        assert!(storage
            .delete_documents(collection_name, vec!["doc3".to_string()])
            .await
            .is_ok());
...
async fn test_error_handling() {
    if let Some(storage) = create_test_storage().await {
        ...
        assert!(storage
            .upsert_documents("nonexistent", vec![])
            .await
            .is_err());
        ...
        assert!(storage
            .upsert_documents(collection_name, vec![invalid_doc])
            .await
            .is_err());
...
async fn test_batch_operations() {
    if let Some(storage) = create_test_storage().await {
        ...
        assert!(storage
            .delete_documents(collection_name, ids_to_delete)
            .await
            .is_ok());
