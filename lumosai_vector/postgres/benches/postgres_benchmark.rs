...
fn generate_documents(count: usize, dimension: usize) -> Vec<Document> {
    (0..count)
        .map(|i| {
            ...
            metadata.insert(
                "category".to_string(),
                MetadataValue::String(format!("cat_{}", i % 10)),
            );
            metadata.insert(
                "score".to_string(),
                MetadataValue::Float((i as f32) / (count as f32)),
            );

            Document {
                id: format!("doc_{}", i),
                content: format!(
                    "This is document number {} with some content for testing",
                    i
                ),
...
fn generate_query_vector(dimension: usize) -> Vec<f32> {
    (0..dimension).map(|i| (i as f32 * 0.1).cos()).collect()
}

async fn setup_storage() -> Result<PostgresVectorStorage> {
    let database_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://postgres:password@localhost/bench_lumos_vector".to_string()
    });
...
fn bench_upsert_documents(c: &mut Criterion) {
    ...
    let storage = rt.block_on(async { setup_storage().await.expect("Failed to setup storage") });
        ...
        group.bench_with_input(
            ...
            |b, docs| {
                b.to_async(&rt).iter(|| async {
                    let result = storage
                        .upsert_documents("bench_index", black_box(docs.clone()))
                        .await;
                    black_box(result)
                });
            },
...
fn bench_search_performance(c: &mut Criterion) {
    ...
    let storage = rt.block_on(async {
        ...
        storage
            .upsert_documents("bench_index", documents)
            .await
