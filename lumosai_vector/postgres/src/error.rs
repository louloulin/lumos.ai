...
impl From<PostgresError> for VectorError {
    fn from(err: PostgresError) -> Self {
        match err {
            ...
            PostgresError::Migration(msg) => {
                VectorError::OperationFailed(format!("Migration: {}", msg))
            }
            PostgresError::Serialization(msg) => VectorError::serialization(msg),
            PostgresError::Config(msg) => {
                VectorError::OperationFailed(format!("Configuration: {}", msg))
            }
            PostgresError::Pool(msg) => VectorError::connection_failed(format!("Pool: {}", msg)),
            PostgresError::Transaction(msg) => {
                VectorError::OperationFailed(format!("Transaction: {}", msg))
            }
            PostgresError::Index(msg) => VectorError::OperationFailed(format!("Index: {}", msg)),
            PostgresError::PgVector(msg) => {
                VectorError::OperationFailed(format!("pgvector: {}", msg))
            }
        }
    }
}
...
impl From<sqlx::Error> for PostgresError {
    fn from(err: sqlx::Error) -> Self {
        match err {
            ...
            sqlx::Error::ColumnIndexOutOfBounds { index, len } => PostgresError::Sql(format!(
            ...
            sqlx::Error::ColumnDecode { index, source } => PostgresError::Serialization(format!(
                "Column decode error at index {}: {}",
                index, source
            )),
...
pub fn is_table_not_found_error(err: &PostgresError) -> bool {
    match err {
        PostgresError::Sql(msg) => msg.contains("relation") && msg.contains("does not exist"),
        _ => false,
    }
}
...
pub fn is_connection_error(err: &PostgresError) -> bool {
    matches!(err, PostgresError::Connection(_) | PostgresError::Pool(_))
}

/// Helper function to create a pgvector extension error
pub fn pgvector_extension_error() -> PostgresError {
    PostgresError::PgVector(
        "pgvector extension is not installed. Please install it with: CREATE EXTENSION vector;"
            .to_string(),
    )
}
