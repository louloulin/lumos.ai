...
use lumosai_vector_postgres::{PostgresConfig, PostgresVectorStorage};
...
async fn test_postgres_basic_operations() -> Result<()> {
    let database_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://postgres:password@localhost/test_lumos_vector".to_string()
    });
    ...
    // Upsert documents
    let ids = storage
        .upsert_documents("test_index", documents.clone())
        .await?;
    assert_eq!(ids.len(), 2);
    ...
    // Get documents
    let retrieved_docs = storage
        .get_documents("test_index", vec!["doc1".to_string()], true)
        .await?;
    ...
    // Delete documents
    storage
        .delete_documents("test_index", vec!["doc1".to_string()])
        .await?;

    // Verify deletion
    let remaining_docs = storage
        .get_documents("test_index", vec!["doc1".to_string()], false)
        .await?;
...
async fn test_postgres_configuration() -> Result<()> {
    let config = PostgresConfig::new("postgresql://localhost/test").with_performance(
        lumosai_vector_postgres::config::PerformanceConfig {
            batch_size: 500,
...
fn test_vector_index_sql_generation() {
    use lumosai_vector_postgres::config::{IndexParams, VectorIndexType};

    let params = IndexParams::default();

    // Test HNSW index SQL
    let hnsw_sql = VectorIndexType::Hnsw.create_index_sql("test_table", "test_idx", &params);
    assert!(hnsw_sql.contains("USING hnsw"));
    assert!(hnsw_sql.contains("vector_cosine_ops"));

    // Test IVFFlat index SQL
    let ivf_sql = VectorIndexType::IvfFlat.create_index_sql("test_table", "test_idx", &params);
    assert!(ivf_sql.contains("USING ivfflat"));
    assert!(ivf_sql.contains("vector_cosine_ops"));

    // Test no index
    let no_index_sql = VectorIndexType::None.create_index_sql("test_table", "test_idx", &params);
