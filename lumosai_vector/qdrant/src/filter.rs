...
use qdrant_client::qdrant::{
    ...
    Range, RepeatedIntegers, RepeatedStrings, Value as QdrantValue,
};
...
impl QdrantFilterConverter {
        ...
        match condition {
            ...
            FilterCondition::In(field, values) => {
                ...
                let match_value = if values.is_empty() {
                    return Err(QdrantError::InvalidFilter(
                        "Empty values list not allowed".to_string(),
                    ));
                } else {
                    // Check the type of the first value to determine the collection type
                    match &values[0] {
                        MetadataValue::String(_) => {
                            let strings: std::result::Result<Vec<String>, QdrantError> = values
                                .into_iter()
                                .map(|v| match v {
                                    MetadataValue::String(s) => Ok(s),
                                    _ => Err(QdrantError::InvalidFilter(
                                        "Mixed types in values list".to_string(),
                                    )),
                                })
                                .collect();
                        ...
                        MetadataValue::Integer(_) => {
                            let integers: std::result::Result<Vec<i64>, QdrantError> = values
                                .into_iter()
                                .map(|v| match v {
                        ...
                        _ => {
                            return Err(QdrantError::InvalidFilter(
                                "Unsupported value type for In filter".to_string(),
                            ))
                        }
                    }
                };
                    ...
                    match &values[0] {
                        ...
                        MetadataValue::Integer(_) => {
                            let integers: std::result::Result<Vec<i64>, QdrantError> = values
                                ...
                                .collect();
                            MatchValue::ExceptIntegers(RepeatedIntegers {
                                integers: integers?,
                            })
                        }
                        _ => {
                            return Err(QdrantError::InvalidFilter(
                                "Unsupported value type for NotIn filter".to_string(),
                            ))
                        }
                    }
                };
            ...
            FilterCondition::Contains(field, text) => Ok(Filter {
                must: vec![Condition {
                    condition_one_of: Some(ConditionOneOf::Field(FieldCondition {
                        key: field,
                        r#match: Some(Match {
    ...
    fn convert_metadata_value(value: MetadataValue) -> QdrantResult<MatchValue> {
        match value {
            ...
            MetadataValue::Array(_) => Err(QdrantError::InvalidFilter(
                "Array values not supported in filters".to_string(),
            )),
            MetadataValue::Object(_) => Err(QdrantError::InvalidFilter(
                "Object values not supported in filters".to_string(),
            )),
            MetadataValue::Null => Err(QdrantError::InvalidFilter(
                "Null values not supported in match filters".to_string(),
            )),
        }
    }
    ...
    fn convert_to_range_value(value: MetadataValue) -> QdrantResult<f64> {
        match value {
            MetadataValue::String(_) => Err(QdrantError::InvalidFilter(
                "String values not supported in range operations".to_string(),
            )),
            MetadataValue::Integer(i) => Ok(i as f64),
            MetadataValue::Float(f) => Ok(f),
            MetadataValue::Boolean(_) => Err(QdrantError::InvalidFilter(
                "Boolean values not supported in range operations".to_string(),
            )),
            MetadataValue::Array(_) => Err(QdrantError::InvalidFilter(
                "Array values not supported in range operations".to_string(),
            )),
            MetadataValue::Object(_) => Err(QdrantError::InvalidFilter(
                "Object values not supported in range operations".to_string(),
            )),
            MetadataValue::Null => Err(QdrantError::InvalidFilter(
                "Null values not supported in range operations".to_string(),
            )),
        }
    }
    ...
    pub fn convert_metadata_to_value(value: MetadataValue) -> QdrantResult<QdrantValue> {
        match value {
            ...
            MetadataValue::Array(_) => Err(QdrantError::Serialization(
                "Array metadata values not supported in payload".to_string(),
            )),
            MetadataValue::Object(_) => Err(QdrantError::Serialization(
...
mod tests {
    ...
    fn test_or_filter() {
        let conditions = vec![
            FilterCondition::Eq(
                "category".to_string(),
                MetadataValue::String("test1".to_string()),
            ),
            FilterCondition::Eq(
                "category".to_string(),
                MetadataValue::String("test2".to_string()),
            ),
        ];
