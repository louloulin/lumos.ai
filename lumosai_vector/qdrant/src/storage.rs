...
impl QdrantVectorStorage {
    ...
    pub async fn with_config(config: QdrantConfig) -> Result<Self> {
        ...
        let client =
            Qdrant::new(qdrant_config).map_err(|e| VectorError::ConnectionFailed(e.to_string()))?;
...
impl VectorStorage for QdrantVectorStorage {
    ...
    async fn upsert_documents(
        ...
        debug!(
            "Upserted {} documents to collection: {}",
            ids.len(),
            collection_name
        );
        Ok(ids)
    ...
    async fn search(&self, request: SearchRequest) -> Result<SearchResponse> {
        ...
        let filter = if let Some(condition) = request.filter {
            Some(
                QdrantFilterConverter::convert_filter(condition)
