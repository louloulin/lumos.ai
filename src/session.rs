...
pub use lumosai_core::agent::session::{
    MemorySessionStorage, SessionData, SessionManager as CoreSessionManager, SessionMetadata,
    SessionQuery, SessionState, SessionStorage, ToolCallHistory, ToolCallStatus,
};
...
/// }
/// ```
pub async fn load(session_id: &str, storage: Arc<dyn SessionStorage>) -> Result<Option<Session>> {
    let manager = CoreSessionManager::new(storage);
...
impl SessionTrait for SimpleSession {
    ...
    async fn add_message(&self, message: Message) -> Result<()> {
        self.manager
            .add_message(&self.data.metadata.session_id, message)
            .await
    }
    ...
    async fn get_messages(&self) -> Result<Vec<Message>> {
        if let Some(session_data) = self
            .manager
            .get_session(&self.data.metadata.session_id)
            .await?
        {
    ...
    async fn set_title(&self, title: &str) -> Result<()> {
        self.manager
            .set_session_title(&self.data.metadata.session_id, title.to_string())
            .await
    }
...
impl SessionBuilder {
    ...
    pub async fn build(self) -> Result<Session> {
        ...
        let session = create_with_storage(&agent_name, self.user_id.as_deref(), storage).await?;
...
mod tests {
    ...
    async fn test_session_creation() {
        let session = create("test_agent", Some("user_123"))
            .await
