...
use lumosai_core::agent::{
    ...
    performance::{PerformanceAnalyzer, PerformanceMonitor},
...
async fn test_api_consistency_check() -> Result<()> {
    ...
    assert!(
        result.score > 0.5,
        "API consistency score should be reasonable"
    );
    assert!(
    ...
    // 检查具体的一致性方面
    let has_critical_issues = result
        .issues
        .iter()
...
async fn test_feature_completeness_check() -> Result<()> {
    ...
    let llm = Arc::new(MockLlmProvider::new(vec![
        ...
        "Step 1: Analyze the input. Step 2: Process the data. Step 3: Generate the result."
            .to_string(),
    ...
    assert!(
        ...
        "Feature completeness score should be reasonable"
    );
    assert!(
        !result.implemented_features.is_empty(),
        "Should have some implemented features"
    );
    assert!(
        !result.recommendations.is_empty(),
...
async fn test_agent_status_management() -> Result<()> {
    ...
    // 测试错误状态
    agent
        .set_status(AgentStatus::Error("Test error".to_string()))
        .unwrap();
...
async fn test_agent_health_check() -> Result<()> {
    ...
    assert!(
        health.contains_key("has_memory"),
        "Should include memory info"
    );
    assert!(
        health.contains_key("tools_count"),
...
async fn test_comprehensive_integration() -> Result<()> {
    ...
    let response = agent
        .generate(&test_message, &Default::default())
        .await
        .unwrap();
...
fn create_echo_tool() -> FunctionTool {
    ...
    FunctionTool::new("echo", "Echoes the input message", schema, |params| {
        let message = params
            .get("message")
            .and_then(|v| v.as_str())
...
fn create_json_tool() -> FunctionTool {
    ...
    FunctionTool::new("json_formatter", "Formats data as JSON", schema, |params| {
