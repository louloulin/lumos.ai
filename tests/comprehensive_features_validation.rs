...
async fn test_config_validation_functionality() {
    ...
    assert!(
        !report.is_valid(),
        "Configuration with issues should not be valid"
    );
...
async fn test_monitoring_functionality() {
    ...
    collector
        .increment_counter("labeled_requests", Some(labels))
        .unwrap();
    ...
    // 测试计时器
    collector
        .record_timer("request_duration", Duration::from_millis(250), None)
        .unwrap();
    collector
        .record_timer("request_duration", Duration::from_millis(180), None)
        .unwrap();

    // 测试直方图
    let response_times = vec![100.0, 150.0, 200.0, 120.0, 180.0];
    collector
        .record_histogram("response_times", response_times, None)
        .unwrap();
...
async fn test_custom_validation_rules() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("Custom field must start with 'custom_'"));
}
