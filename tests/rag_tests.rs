...
async fn test_vector_storage_crud() {
    ...
    storage
        .create_index(
            "test",
            384,
            Some(lumosai_core::vector::SimilarityMetric::Cosine),
        )
        .await
        .unwrap();
    ...
    // 查询向量
    let results = storage
        .query("test", vec![0.1; 384], 5, None, true)
        .await
        .unwrap();
...
async fn test_vector_similarity_metrics() {
    ...
    let _ids = storage
        .upsert("cosine_test", vectors, None, None)
        .await
        .unwrap();

    // 查询相似向量
    let results = storage
        .query("cosine_test", vector3, 2, None, true)
        .await
        .unwrap();
...
async fn test_vector_storage_performance() {
    ...
    let _ids = storage
        .upsert("perf_test", vectors, None, None)
        .await
        .unwrap();
    ...
    let start_time = Instant::now();
    let results = storage
        .query("perf_test", query_vector, 10, None, true)
        .await
        .unwrap();
...
async fn test_vector_storage_concurrency() {
    ...
    let tasks: Vec<_> = (0..5)
        .map(|i| {
            ...
            tokio::spawn(async move {
                ...
                storage_clone
                    .upsert("concurrent_test", vectors, None, None)
                    .await
            })
        })
        .collect();
    ...
    let query_results = storage
        .query("concurrent_test", vec![0.0; 384], 10, None, true)
        .await
        .unwrap();
...
async fn test_rag_query_accuracy() {
    ...
    let _ids = storage
        .upsert("accuracy_test", doc_embeddings, None, None)
        .await
        .unwrap();
    ...
    let results = storage
        .query("accuracy_test", query_embedding, 3, None, true)
        .await
        .unwrap();
...
async fn test_rag_ranking_quality() {
    ...
    let _ids = storage
        .upsert("ranking_test", vectors, None, None)
        .await
        .unwrap();
    ...
    let results = storage
        .query("ranking_test", query, 3, None, true)
        .await
        .unwrap();
...
async fn test_rag_context_window() {
    ...
    let _ids = storage
        .upsert("context_test", vectors, None, None)
        .await
        .unwrap();
    ...
    for k in [1, 5, 10, 15] {
        let results = storage
            .query("context_test", vec![0.5; 384], k, None, true)
            .await
            .unwrap();
